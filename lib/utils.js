...
/** outputDefinitionに対応する要素を取得するためのqueryを作成する */
export function createQueryForOutputDefinition(od) {
  return `[name="${od.getName()}"]`;
}

/**
 * outputDefinitionsに対応する要素を取得する
 */
export function findElementsByOutputDefinitions(...outputDefinitions) {
  const query = outputDefinitions.map(od => createQueryForOutputDefinition(od)).join(',');
  return $(query);
}

/** output-noを表示する */
export function showOutputNo(survey) {
  survey.getAllOutputDefinitions().forEach((od) => {
    const $targetElements = findElementsByOutputDefinitions(od);
    if ($targetElements.prop('type') === 'radio') {
      $targetElements.each((i, el) => {
        const $el = $(el);
        const value = $el.val();
        const $insertEl = $('<span class="output-no" />').text(`${od.getOutputNo()}(${value})`);
        $el.after($insertEl);
      });
    } else {
      const $insertEl = $('<span class="output-no" />').text(od.getOutputNo());
      $targetElements.after($insertEl);
    }
  });
}
...
/**
 * 縦書き対応時に不具合のある文字を変換する
 */
export function convertVerticalWriting(className, text) {
  if (className !== 'vertical-writing') return text;
  // IEにおける全角ハイフン（#xFF0D）を変換する
 ...
 * 全角の数値を半角の数値に置き換える
 */
export function zenkakuNum2Hankaku(str) {
  return str.replace(/[０-９]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 65248));
}

/**
 * URLにパラメータを付ける
 */
export function appendUrlParams(url, paramStr) {
  if (url.indexOf('?') === -1) {
    return `${url}?${paramStr}`;
  }
  return `${url}&${paramStr}`;
