...
export default class ReplaceUtil {
   ...
   * コンストラクタ
   *
   * @param {*} outputDefinitionMap 出力定義
   */
  ...
  name2OutputNo(str) {
    ...
    this.allOutputDefinitionMap.forEach((value, key) => {
      ...
      encodedStr = encodedStr.replace(pattern, (match, questionNo, prop) => `\${Q${od.getOutputNo()}#${prop}}`);
    });
    return encodedStr;
  }
}
