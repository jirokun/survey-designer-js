...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  getQuestionNoMap() {
    ...
    this.getPages().forEach((page) => {
      page.getQuestions().forEach((question) => {
        question.getOutputDefinitions().forEach((od) => {
          const questionNo = this.calcQuestionNo(page.getId(), question.getId(), od.getPostfix());
          questionNoMap[questionNo] = od.getName();
        });
      });
    });
  ...
  getQuestionNameMap() {
    ...
    this.getPages().forEach((page) => {
      page.getQuestions().forEach((question) => {
        question.getOutputDefinitions().forEach((od) => {
          const questionNo = this.calcQuestionNo(page.getId(), question.getId(), od.getPostfix());
          questionNameMap[od.getName()] = questionNo;
        });
      });
    });
  ...
  getAllOutputDefinitionMap() {
    let map = Map();
    const allOutputDefinitions = this.getAllQuestions().flatMap(question => question.getOutputDefinitions()).toList();
