...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  deleteBranch(branchId) {
    return this.setIn(['defs', 'branchDefs'], this.getBranchDefs().filter(branch => branch.getId() !== branchId));
  }

  /** pageを削除する */
  deletePage(pageId) {
    return this.setIn(['defs', 'pageDefs'], this.getPageDefs().filter(page => page.getId() !== pageId));
  }
  ...
  deleteFlow(flowId) {
    ...
    const flowDeletedState = refDeletedState.setIn(['defs', 'flowDefs'], this.getFlowDefs().filter(f => f.getId() !== flowId));
    // currentFlowIdも更新
