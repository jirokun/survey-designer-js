...
import PersonalQuestionItemDefinition from './survey/questions/internal/PersonalQuestionItemDefinition';
import PersonalItemDisplayTypeDefinition from './survey/questions/internal/PersonalItemDisplayTypeDefinition';
import PersonalItemFieldDefinition from './survey/questions/internal/PersonalItemFieldDefinition';
  ...
  static createFromJson(json, opts = {}) {
    let parsedObj = fromJS(json, (key, value) => {
      switch (key) {
        ...
        case 'items':
          return value.map((v) => {
            if (v.get('dataType') === 'PersonalItem') { return new PersonalQuestionItemDefinition(v); }
        ...
        case 'personalItemDisplayTypeCandidates':
          return value.map(v => new PersonalItemDisplayTypeDefinition(v));
        case 'personalItemFields':
          return value.map(v => new PersonalItemFieldDefinition(v));
        default:
          return value;
      }
    });
