...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  /** logicVariableを削除する */
  removeLogicVariable(pageId, logicVariableId) {
    const pageIndex = this.findPageIndex(pageId);
    return this.updateIn(['survey', 'pages', pageIndex, 'logicVariables'], logicVariables => logicVariables.filter(lv => lv.getId() !== logicVariableId));
  }
  ...
  removeNode(nodeId) {
    const node = this.findNode(nodeId);
    let refDeletedState;
    if (node.isPage()) refDeletedState = this.removePage(node.getRefId());
    else if (node.isBranch()) refDeletedState = this.removeBranch(node.getRefId());
    else if (node.isFinisher()) refDeletedState = this.removeFinisher(node.getRefId());
  ...
  /** Conditionを削除する */
  removeCondition(branchId, conditionId) {
    const branchIndex = this.findBranchIndex(branchId);
    const branch = this.findBranch(branchId);
  ...
  submitPage(pageAnswers) {
    ...
    return this.update('runtime', (runtime) => {
      const newRuntime = runtime.mergeDeep({ answers: pageAnswers });
      if (nextNode.isPage() || nextNode.isFinisher()) {
        ...
        const nextNodeId = branch.evaluateConditions(newRuntime.getAnswers(), this.getAllOutputDefinitionMap())
  ...
  /** pageの属性の更新 */
  updatePageAttribute(pageId, attributeName, value) {
    const pageIndex = this.findPageIndex(pageId);
    return this.setIn(['survey', 'pages', pageIndex, attributeName], value);
  }

  /** logicVariableの更新 */
  updateLogicVariable(pageId, logicVariableId, logicVariable) {
    const pageIndex = this.findPageIndex(pageId);
    const logicVariableIndex = this.findPage(pageId).findLogicVariableIndex(logicVariableId);
    return this.setIn(['survey', 'pages', pageIndex, 'logicVariables', logicVariableIndex], logicVariable);
  }
  ...
  updateQuestionAttribute(pageId, questionId, attributeName, value1, value2) {
    const pageIndex = this.findPageIndex(pageId);
    const questionIndex = this.findQuestionIndex(pageId, questionId);
    const ru = this.getReplaceUtil();
    if (attributeName === 'title') {
      const newState = this.setIn(['survey', 'pages', pageIndex, 'questions', questionIndex, attributeName], ru.outputNo2Name(value1))
        .setIn(['survey', 'pages', pageIndex, 'questions', questionIndex, 'plainTitle'], ru.outputNo2Name(value2));
      return newState;
    }
    if (attributeName === 'items') {
      throw new Error('updateQuestionAttributeではitemsを更新できません');
    }
    return this.setIn(['survey', 'pages', pageIndex, 'questions', questionIndex, attributeName], ru.outputNo2Name(value1));
  }
  ...
  updateItemAttribute(pageId, questionId, itemId, attributeName, value) {
    ...
    const ru = this.getReplaceUtil();
    return this.setIn(['survey', 'pages', pageIndex, 'questions', questionIndex, 'items', itemIndex, attributeName], ru.outputNo2Name(value));
  }
  ...
  updateFinisherAttribute(finisherId, attributeName, value) {
    ...
    const ru = this.getReplaceUtil();
    return this.setIn(['survey', 'finishers', finisherIndex, attributeName], ru.outputNo2Name(value));
  }
  ...
  /** LogicVariableを追加する */
  addLogicVariable(pageId) {
    const pageIndex = this.findPageIndex(pageId);
    const page = this.findPage(pageId);
    const logicVariable = LogicVariableDefinition.create();
    const pageNo = this.calcPageNo(pageId);
    let name;
    for (let i = 0; i < 1000; i++) {
      name = `${pageNo}-L-${S(i).padLeft(3, '0')}`;
      if (page.isUniqueLogicVariableName(logicVariable.getId(), name)) break;
    }
    return this.updateIn(['survey', 'pages', pageIndex, 'logicVariables'], logicVariables => logicVariables.push(logicVariable.set('variableName', name)));
  }
  ...
  findPrecedingOutputDefinition(nodeId) {
    const precedingPageNodeIds = this.findPrecedingPageNodeIds(nodeId);
  ...
  /** ReplaceUtilを生成して返す */
  getReplaceUtil() {
    return new ReplaceUtil(this.getAllOutputDefinitionMap(), this.getQuestionNoMap(), this.getAnswers());
  }
