...
import ItemDefinition from './definitions/questions/ItemDefinition';
...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  /** 回答の提出状態を更新する */
  updateAnswerPostStatus(postStatus) {
    return this.setIn(['runtime', 'postAnswerStatus'], postStatus);
  }
  ...
  swapQuestion(srcNodeId, srcQuestionId, destNodeId, destQuestionId) {
    const srcPage = this.findPageFromNode(srcNodeId);
    const destPage = this.findPageFromNode(destNodeId);
    const srcPageIndex = this.findPageIndex(srcPage.getId());
    const destPageIndex = this.findPageIndex(destPage.getId());
    const srcQuestionIndex = this.findQuestionIndex(srcPage.getId(), srcQuestionId);
    const destQuestionIndex = this.findQuestionIndex(destPage.getId(), destQuestionId);
    const srcQuestion = this.findQuestion(srcQuestionId);
    const destQuestion = this.findQuestion(destQuestionId);

    return this
      .setIn(['survey', 'pages', destPageIndex, 'questions', destQuestionIndex], srcQuestion)
      .setIn(['survey', 'pages', srcPageIndex, 'questions', srcQuestionIndex], destQuestion);
  }
  ...
  /**
   * すべての設問のOutputDefinitionをnameをkeyにして返す
   */
  getAllOutputDefinitionMap() {
    ...
    allOutputDefinitionsList.forEach((outputDefinition) => {
      const outputName = outputDefinition.getName();
      const value = map.get(outputName);
      if (value) {
        throw new Error(`outputNameが重複しています。outputName: ${outputName}`);
      }
      map = map.set(outputName, outputDefinition);
    });
    return map;
  }
