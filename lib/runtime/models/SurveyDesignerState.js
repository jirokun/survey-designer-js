...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  /** questionの属性の更新 */
  updateQuestionAttribute(pageId, questionId, attributeName, html, text) {
    const pageIndex = this.getPageDefs().findIndex(p => p.getId() === pageId);
    const questionIndex = this.findPage(pageId).getQuestions().findIndex(q => q.getId() === questionId);
    if (attributeName === 'title') {
      const newState = this.setIn(['defs', 'pageDefs', pageIndex, 'questions', questionIndex, attributeName], html)
        .setIn(['defs', 'pageDefs', pageIndex, 'questions', questionIndex, 'plainTitle'], text);
      return newState;
    }
    return this.setIn(['defs', 'pageDefs', pageIndex, 'questions', questionIndex, attributeName], html);
  }
  ...
  addQuestion(questionClassName, pageId, index) {
    const question = QuestionDefinition.create(questionClassName);
