...
export default class SurveyDesignerState extends SurveyDesignerStateRecord {
  ...
  static createFromJson(json, opts = {}) {
    let parsedObj = fromJS(json, (key, value) => {
      switch (key) {
        ...
        case 'cssOptions':
          return value.map((v) => CssOption.create(v.get('title'), v.get('runtimeUrls'), v.get('previewUrls'))).toList();
        default:
          return value;
      }
    });
    ...
    if (survey) {
      parsedObj = parsedObj.toMap().set('survey', survey.migrate());
    }
  ...
  setInitialCssUrls() {
    if (this.getSurvey().hasCssUrls() || !this.getOptions().hasCssOptions()) {
      return this;
    }

    const runtimeUrls = this.getOptions().getCssOptions().first().getRuntimeUrls();
    const previewUrls = this.getOptions().getCssOptions().first().getPreviewUrls();
    return this.update('survey', survey => survey.updateCssUrls(runtimeUrls, previewUrls));
  }
