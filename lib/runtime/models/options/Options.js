...
export const OptionsRecord = Record({
  ...
  exposeSurveyManager: false,              // SurveyManagerをwindowに紐つけるかどうか。trueの場合はsmという名前で設定する。文字列の場合は指定した文字列の名前で設定する。
});
...
export default class Options extends OptionsRecord {
  ...
  isExposeSurveyManager() {
    return this.get('exposeSurveyManager');
  }
}
