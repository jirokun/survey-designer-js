...
export default class Options extends OptionsRecord {
  ...
  getCurrentCssTitle(urls) {
    let ret = null;
    this.getCssOptions().forEach(function (option) {
      if (option.get('urls').toArray().sort().toString() === urls.toArray().sort().toString()) {
        ret = option.get('title');
      }
    });
    return ret;
  }

  getCssUrlsByTitle(title) {
    let ret = null;
    this.getCssOptions().forEach(function (option) {
