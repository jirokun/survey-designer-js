...
export default class MultiNumberQuestionDefinition extends BaseQuestionDefinition {
  ...
  /** 出力に使用する名前を取得する */
  getOutputName(position, index = null) {
    switch (position) {
      case 'additional':
  ...
  getOutputDefinitions(pageNo, questionNo) {
    ...
    this.getItems().forEach((item) => {
      const name = this.getOutputName(null, item.getIndex());
      ...
      if (item.hasAdditionalInput()) {
        outputDefinitions = outputDefinitions.push(new OutputDefinition({
          ...
          questionId: this.getId(),
          name: this.getOutputName('additional', item.getIndex()),
    ...
    // 合計値
    if (this.showTotal) {
      const name = this.getOutputName('total');
