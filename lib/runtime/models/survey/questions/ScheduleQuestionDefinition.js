...
export default class ScheduleQuestionDefinition extends BaseQuestionDefinition {
  static create(options) {
    ...
    const items = List([
      ...
      return ItemDefinition.create(itemId, index)
          .set('label', label)
    ...
    /** 出力に使用する名前を取得する */
  getOutputName(other, itemId = null, periodNo = null) {
    if (other) return `${this.getId()}_other`;
    return `${this.getId()}_${itemId}_period${periodNo}__text`;
  }
  ...
  getOutputDefinitions(pageNo, questionNo) {
    ...
    this.getItems().forEach((item, itemIndex) => {
      periods.forEach((period, periodIndex) => {
        const outputDefinitionId = this.getOutputName(false, item.getId(), period.no);
        outputDefinitions.push(new OutputDefinition({
          ...
          dlLabel: `${item.getPlainLabel()} ${period.label}`,
          question: this,
          outputType: 'text',
