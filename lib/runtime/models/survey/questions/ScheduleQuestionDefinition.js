...
export default class ScheduleQuestionDefinition extends BaseQuestionDefinition {
  static create(pageDevId) {
    ...
    let question = new ScheduleQuestionDefinition({
      _id: cuid(),
      devId: questionDevId,
      dataType: 'Schedule',
    ...
    question = question.updateDefaultItems();
    return question.updateDefaultSubItems();
  }

  updateDefaultItems() {
    return this.set('items', List([
      '9月2日(月)',
      '9月3日(火)',
    ]).map((label, index) => {
      const itemDevId = surveyDevIdGeneratorInstance.generateForItem(this.getDevId());
  ...
  updateDefaultSubItems() {
    return this.set('subItems', List([
      { label: '<b>A.<br />午前<br />9:00～12:00</b>', plainLabel: 'A.午前9:00～12:00' },
      { label: '<b>B.<br />午後<br />12:00～16:00</b>', plainLabel: 'B.午後12:00～16:00' },
      { label: '<b>C.<br />夜間<br />16:00 以降</b>', plainLabel: 'C.夜間16:00 以降' },
    ]).map((e, index) => {
      const itemDevId = surveyDevIdGeneratorInstance.generateForItem(this.getDevId());
      return ItemDefinition.create(itemDevId, index)
        .set('label', e.label)
  ...
  getOutputDefinitions(pageNo = '0', questionNo = '0') {
    ...
    // 「上記のいずれも都合がつかない」の分
    outputDefinitions.push(this.getOtherOutputDefinition(pageNo, questionNo));

    return List(outputDefinitions);
  }

  /** 「上記のいずれも都合がつかない」のOutputDefinitionを返す */
  getOtherOutputDefinition(pageNo = '0', questionNo = '0') {
  ...
  /** OutputDefinitionの要素に対応するチェックボックスのIDを取得する */
  getToggleCheckboxIdByOutputDefinition(outputDefinition) {
    return `${outputDefinition.getId()}_checkbox`;
  }
}
