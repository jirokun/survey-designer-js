...
export default class MatrixQuestionDefinition extends BaseQuestionDefinition {
  ...
  getOutputDevId(rowDevId, columnDevId, isAdditional) {
    if (rowDevId == null || columnDevId == null) { // devIdが指定されていない場合はnullを返す
      return null;
    }

    const columnLastDevId = columnDevId.split('_').slice(-1)[0];
    return isAdditional ? `${rowDevId}_${columnLastDevId}_text` : `${rowDevId}_${columnLastDevId}`;
  }
    ...
    return columns.filter(item => item.hasAdditionalInput()).flatMap(column =>
      rows.map((row) => {
        ...
        return new OutputDefinition({
          ...
          questionId: this.getId(),
          devId: this.getOutputDevId(row.getDevId(), column.getDevId(), true),
