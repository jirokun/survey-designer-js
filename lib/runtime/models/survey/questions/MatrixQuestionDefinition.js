...
export default class MatrixQuestionDefinition extends BaseQuestionDefinition {
  ...
  getOutputDefinitionForAdditionalInput(pageNo, questionNo) {
    ...
    return rows.filter(item => item.hasAdditionalInput()).map((row) => {
      const label = `${row.getPlainLabel()}-入力欄`;
      const dlLabel = `${this.getPlainTitle()}-${row.getPlainLabel()}-入力欄`;
      return new OutputDefinition({
        _id: `${row.getId()}_additional_input`,
        questionId: this.getId(),
        devId: `${row.getDevId()}_text`,
        name: `${this.getId()}_value${row.getIndex() + 1}__text`,
        label,
        dlLabel,
        question: this,
        outputType: row.getAdditionalInputType(),
        outputNo: BaseQuestionDefinition.createOutputNo(pageNo, questionNo, row.getIndex() + 1, 'additional'),
      });
    });
  }
