...
export default class MatrixQuestionDefinition extends BaseQuestionDefinition {
  ...
  getOutputDefinitionsCommon(pageNo, questionNo) {
    ...
    if (this.isMatrixReverse()) {
      outputDefinitions = this.getSubItems().flatMap(subItem =>
        this.getItems().map(item => this.getOutputDefinitionFromItemAndSubItem(item, subItem, pageNo, questionNo)),
      );
    } else {
      outputDefinitions = this.getItems().flatMap(item =>
  ...
  validate(survey) {
    ...
    if (!replacer.validate(this.getItems().map(item => item.getLabel()).join(''), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('行項目で存在しない参照があります'));
    if (!replacer.validate(this.getSubItems().map(subItem => subItem.getLabel()).join(''), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('列項目で存在しない参照があります'));
    ...
    // itemIdの行または列がない場合警告
    if (page.isFreeMode() || (this.isMatrixHtmlEnabled() && !S(this.getMatrixHtml()).isEmpty())) {
      const $html = $(page.isFreeMode() ? page.getHtml() : this.getMatrixHtml());
      // 行
