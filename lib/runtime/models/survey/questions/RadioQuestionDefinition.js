...
export default class RadioQuestionDefinition extends BaseQuestionDefinition {
  static create(options) {
    ...
    const itemId = surveyIdGeneratorInstance.generateQuestionId(options.pageId, questionId);
  ...
  /** 出力に使用する名前を取得する */
  getOutputName(additionalInput = null, itemId = null) {
    if (additionalInput) return `${this.getId()}_${itemId}`;
    return `${this.getId()}`;
  }
  ...
  getOutputDefinitions(pageNo, questionNo) {
    ...
    const outputName = this.getOutputName(false);
    ...
    this.getItems().forEach((item) => {
      ...
      const additionInputId = this.getOutputName(true, item.getId());
      outputDefinitionArray.push(new OutputDefinition({
        _id: item.getId(),
        questionId: this.getId(),
