...
export default class VisibilityConditionDefinition extends VisibilityConditionDefinitionRecord {
  ...
  validate(survey, node, page, question, item) {
    ...
    if (!outputDefinition) {
      errors = errors.push(ValidationErrorDefinition.createError(`${location}条件設問の値が不正です`));
      return errors;
    }
    ...
    if (outputType === 'number' && S(comparisonType).isEmpty()) errors = errors.push(ValidationErrorDefinition.createError(`${location}比較値タイプが不正です`));
    if (comparisonType === 'fixedValue' && !S(value).isNumeric()) {
      errors = errors.push(ValidationErrorDefinition.createError(`${location}比較値が不正です`));
    ...
    if (S(this.getOperator()).isEmpty()) errors = errors.push(ValidationErrorDefinition.createError(`${location}比較方法が不正です`));
    if (S(this.getVisibilityType()).isEmpty()) errors = errors.push(ValidationErrorDefinition.createError(`${location}動作種別が不正です`));
    return errors;
  }
}
