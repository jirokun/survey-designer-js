...
export const BaseQuestionDefinitionRecord = Record({
  ...
  matrixTategakiCols: false,                       // Matrixの行ヘッダを縦書きするか
...
export default class BaseQuestionDefinition extends BaseQuestionDefinitionRecord {
  ...
  isMatrixTategakiCols() { return this.get('matrixTategakiCols'); }
  ...
  getUniquedNumberValidationRules() {
    ...
    return numberValidationRuleMap
      .keySeq()
      .map(key => this.findNumberValidationRuleFromOutputDefinitionId(key))
      .toSet()
      .toList()
      .sortBy(nvr => nvr.validationTypeInQuestion);
  }
  ...
  updateNumberValidationTypeInQuestion(targetOutputDefinitionId) {
    ...
    outputDefinitions.forEach((od) => {
      ...
      typeInQuestionMap = typeInQuestionMap.set(numberValidationRule, numberValidationRule);
    ...
    if (typeInQuestionMap.includes(targetNumberValidationRule)) {
      const originalNumberValidationRule = typeInQuestionMap.get(targetNumberValidationRule);
