...
export const BaseQuestionDefinitionRecord = Record({
  ...
  matrixHtmlEnabled: false,                        // 表形式をHTMLで表示するかどうか
  matrixHtml: null,                                // 表形式のHTML。これが設定されていればmatrixの表はこのHTMLで表示する
  matrixRowAndColumnUnique: false,                 // 縦方向にユニークにする制約を入れるか(ランキング設問に対応)
...
export default class BaseQuestionDefinition extends BaseQuestionDefinitionRecord {
  ...
  isOptional() { return this.get('optional'); }
  ...
  isMatrixHtmlEnabled() { return this.get('matrixHtmlEnabled'); }
  getMatrixHtml() { return this.get('matrixHtml'); }
  isMatrixRowAndColumnUnique() { return this.get('matrixRowAndColumnUnique'); }
  ...
  validate(survey) {
    ...
    if (!replacer.validate(this.getTitle(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('タイトルで存在しない参照があります'));
    if (!replacer.validate(this.getDescription(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('補足で存在しない参照があります'));
    if (!replacer.validate(this.getUnit(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('単位で存在しない参照があります'));
    if (!replacer.validate(this.getMin(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('最小値で存在しない参照があります'));
    if (!replacer.validate(this.getMax(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('最大値で存在しない参照があります'));
    if (!replacer.validate(this.getTotalEqualTo(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('合計値で存在しない参照があります'));
    if (this.isMatrixHtmlEnabled() && !replacer.validate(this.getMatrixHtml(), outputDefinitions)) errors = errors.push(ValidationErrorDefinition.createError('表形式のテーブルカスタマイズで存在しない参照があります'));
