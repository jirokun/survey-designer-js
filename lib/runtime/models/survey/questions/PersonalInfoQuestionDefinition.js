...
export default class PersonalInfoQuestionDefinition extends BaseQuestionDefinition {
  ...
  getOutputDefinitions() {
    ...
    const outputDefinitionArray = [
        ...
        { id: FIELDS.hospitalPrefecture, label: '病院所在地', outputType: 'select', overrideItems: PREFECTURES },
        ...
        { id: FIELDS.sex, label: '性別', outputType: 'radio', overrideItems: ['男', '女'] },
      ...
      _id: `${id}_${this.getOutputName(item.id)}`, // 並び替えがないので_idとnameは同一で構わない
      name: `${id}_${this.getOutputName(item.id)}`,
