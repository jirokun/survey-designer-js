...
export default class PersonalInfoQuestionDefinition extends BaseQuestionDefinition {
  ...
  getOutputDefinitions(pageNo, questionNo) {
    const outputDefinitionArray = [
        ...
        { id: FIELDS.hospitalPrefecture, label: '病院所在地', outputType: 'select', choices: PREFECTURES },
        ...
        { id: FIELDS.sex, label: '性別', outputType: 'radio', choices: ['男', '女'] },
        { id: FIELDS.mobileTel, label: '携帯電話番号', outputType: 'text' },
        { id: FIELDS.homeTel, label: 'TEL（自宅）', outputType: 'text' },
        { id: FIELDS.workTel, label: 'TEL（勤務先）', outputType: 'text' },
      ...
      const choices = item.outputType === 'radio' || item.outputType === 'select' ?
        List(item.choices.map((choice, j) => new ChoiceDefinition({
          _id: `${this.getId()}_${j + 1}`,
          label: choice,
          value: choice,
        }))) : null;
