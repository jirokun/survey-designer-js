...
import PersonalItemDefinition from './internal/PersonalItemDefinition';
...
export default class PersonalInfoQuestionDefinition extends BaseQuestionDefinition {
  static create(pageDevId) {
        ...
        displayTypeCandidates: [
          {
            ...
            label: 'チェックボックス',
            fields: [
              { id: FIELDS.interviewMeans, label: 'ご連絡方法', outputType: 'checkbox' },
            ],
          },
          { id: 'none', label: '非表示' },
        ],
    ...
    ]).map((item, index) => PersonalItemDefinition.create(
      index,
      item.rowType,
      item.label,
      item.isRequire,
