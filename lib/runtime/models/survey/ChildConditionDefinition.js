...
export default class ChildConditionDefinition extends ChildConditionDefinitionRecord {
  ...
  validate(survey, branchId) {
    ...
    if (!allOutputDefinitionMap.has(this.getOutputId())) {
      errors = errors.push(ValidationErrorDefinition.createError('設定されていない分岐条件があります'));
    } else {
      ...
      if (od.getOutputType() === 'number' && this.getValue() === '') {
        errors = errors.push(ValidationErrorDefinition.createError('分岐条件の入力値が空欄です'));
      } else if (od.isOutputTypeSingleChoice() && this.getValue() === '') {
        errors = errors.push(ValidationErrorDefinition.createError('分岐条件の入力値が選択されていません'));
      } else if (
        (od.getOutputType() === 'number' || od.isOutputTypeSingleChoice())
        && !replacer.validate(this.getValue(), precedingOutputDefinitions)
      ) {
        errors = errors.push(ValidationErrorDefinition.createError('分岐条件の参照先が存在しません'));
      }
    }
    return errors;
  }
