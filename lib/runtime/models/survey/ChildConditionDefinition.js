...
export default class ChildConditionDefinition extends ChildConditionDefinitionRecord {
  ...
  validate(survey, branchId) {
    ...
    if (!allOutputDefinitionMap.has(this.getOutputId())) {
      ...
      if (od.getOutputType() === 'number' && this.getValue() === '') errors = errors.push('分岐条件の入力値が空欄です');
      else if (od.getOutputType() === 'number' && !replacer.validate(this.getValue(), precedingOutputDefinitions)) errors = errors.push('分岐条件の参照先が存在しません');
      else if (od.getOutputType() === 'radio' && this.getValue() === '') errors = errors.push('分岐条件の入力値が選択されていません');
    }
    return errors;
  }
}
