...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  findNextPageIdFromRefId(refId) {
    const node = this.findNodeFromRefId(refId);
    if (node == null) {
      return null;
    }
  ...
  /** JavaScriptの一括更新 */
  updateAllJavaScript(idAndJs,savedAt) {
    let newSurvey = this;
    idAndJs.entrySeq().forEach(function (e) {
      const key = e[0];
      const ids = newSurvey.getPages().map(e => e.getId());
      const index = newSurvey.findPageIndex(key);
      const javaScriptCode = e[1];
      newSurvey = newSurvey.updateIn(['pages', index], page => page.updatePageAttribute('javaScriptCode', javaScriptCode));
    });
    return newSurvey;
  }
   ...
   */
  getAllPageDevIds() {
    return this.getPages().map(page => page.getDevId());
  }
  ...
  getAllQuestionDevIds() {
    return this.getAllQuestions()
      ...
      .filter(id => id !== undefined); // 古いバージョンではidが割り振られていないため無視
  }
  ...
  getAllSubItemDevIds() {
    return this.getAllQuestions().flatMap((question) => {
      return question.getSubItems().map(item => item.getDevId()).filter(id => id !== undefined);
    });
  }
