...
import { migrateNumberValidation } from '../migrator/20170828003100_migrateNumberValidation';
import { migrateScheduleSubItems } from '../migrator/20170921104000_migrateScheduleSubItems';
import { migrateCssUrls } from '../migrator/20171002000000_migrateCssUrls';
import { migratePersonalInfoQuestion } from "../migrator/20171018000000_migratePersonalInfoQuestion";
...
export const SurveyDefinitionRecord = Record({
  ...
  cssPreviewUrls: List(),
  cssDetailUrls: List(),
  zeroSetting: true,   // 検証エラー時に数値の未入力フィールドに0を埋めるかどうか。true:有効 false:無効。ただし各PageDefinitionの定義が優先
});
...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  getCssRuntimeUrls() {
    return this.get('cssRuntimeUrls');
  }

  getCssPreviewUrls() {
    return this.get('cssPreviewUrls');
  }

  getCssDetailUrls() {
    return this.get('cssDetailUrls');
  }

  getZeroSetting() {
    /*
    // 過去データのためundefinedの場合はtrueを返す
    const zeroSetting = this.get('zeroSetting');
    if (zeroSetting === undefined) return true;
    return zeroSetting;
    */
  ...
  findQuestionIndex(questionId) {
    return this.getPages()
      ...
      .findIndex(question => question.getId() === questionId);
  }
  ...
  updateAllJavaScriptCode(allJavaScriptCode) {
    ...
    this.getPages().forEach((page) => {
      const code = allJavaScriptCode.getCodeByDevId(page.getDevId());
      if (code !== null) {
        const index = newSurvey.findPageIndex(page.getId());
        newSurvey = newSurvey.updateIn(['pages', index], p => p.updatePageAttribute('javaScriptCode', code));
      }
    });
    return newSurvey;
  }
  ...
  /** cssUrlsの更新 */
  updateCssUrls(cssRuntimeUrls, cssPreviewUrls, cssDetailUrls) {
    return this.merge({ cssRuntimeUrls, cssPreviewUrls, cssDetailUrls });
  }
  ...
  /** Replacerを生成して返す */
  refreshReplacer(answers = {}) {
    this.replacerCache = new Replacer(this.getAllOutputDefinitionMap(), answers);
  ...
  getAllOutputDefinitions() {
    ...
    this.allOutputDefinitionsCache = this.getPages().flatMap(page => page.getOutputDefinitions(this)).toList();
  ...
  getAllQuestionDevIds() {
    return this.getAllQuestions()
      ...
      .filter(Boolean); // 古いバージョンではidが割り振られていないため無視
  }
  ...
  getAllSubItemDevIds() {
    return this.getAllQuestions().flatMap((question) => {
      return question.getSubItems().map(item => item.getDevId()).filter(Boolean);
    });
  }
  ...
  findPrecedingOutputDefinition(nodeId, includeCurrentPage = true, includeCurrentPageLogicalVariables = true) {
    // precedingOutputDefinitionCacheはSurveyが更新されていればSurveyのインスタンスが異なるのでキャッシュしても問題ない
    if (!this.precedingOutputDefinitionCache) this.precedingOutputDefinitionCache = {};

    const key = `${nodeId}-${includeCurrentPage}-${includeCurrentPageLogicalVariables}`;
    if (this.precedingOutputDefinitionCache[key]) return this.precedingOutputDefinitionCache[key];

    const precedingPageNodeIds = this.findPrecedingPageNodeIds(nodeId, includeCurrentPage);
    const result = List(precedingPageNodeIds).flatMap((nodeId2) => {
    ...
    this.precedingOutputDefinitionCache[key] = result;
    return result;
  }
  ...
  /** numberValidationRuleIdから対応するnumberValidationRuleを取得する */
  findNumberValidationRuleById(numberValidationRuleId) {
    const questions = this.getPages().flatMap(page => page.getQuestions());
    const targetNumberValidationRule = questions
      .flatMap(question => question.getNumberValidationRuleMap().valueSeq())
      .map(numberValidationRuleList => numberValidationRuleList.get(0))
      .find(numberValidationRule => numberValidationRule.getId() === numberValidationRuleId);
    return targetNumberValidationRule;
  }
  ...
  validate() {
    ...
    // パネルが選択されている必要がある
    if (!panel) errors = errors.push(ValidationErrorDefinition.createError('パネルが選択されていません'));
    // 最後のnodeはfinisherである
    if (!this.getNodes().last().isFinisher()) {
      errors = errors.push(ValidationErrorDefinition.createError('最後のページが終了ページではありません'));
    } else if (!this.isValidFinisher()) { // Finisher以降にページが存在しない
      errors = errors.push(ValidationErrorDefinition.createError('終了ページより後ろに「ページ、分岐ページ」を設置しないで下さい'));
    } else if (!this.isValidPositionOfCompleteFinisher()) { // 最後のページの次はCOMPLETEのFinisherである
      errors = errors.push(ValidationErrorDefinition.createError('事故防止のため、最後の「ページ、分岐ページ」の次はCOMPLETEの終了ページにして下さい'));
    }

    // page, branch, finisherのvalidation
    errors = errors.concat(this.getPages().flatMap(page => page.validate(this)));
    errors = errors.concat(this.getBranches().flatMap(branch => branch.validate(this).map(error => new ValidationErrorDefinition({ type: error.getType(), message: `分岐 ${this.calcBranchNo(branch.getId())} ${error.getMessage()}` }))));
    errors = errors.concat(this.getFinishers().flatMap(finisher => finisher.validate(this).map(error => new ValidationErrorDefinition({ type: error.getType(), message: `終了 ${this.calcFinisherNo(finisher.getId())} ${error.getMessage()}` }))));
  ...
  /** 次のページ以外に遷移する分岐があるか */
  hasNotDefaultTransitionBranch() {
    return !!this.getBranches().find(branch => branch.getNextNodeId() !== null);
  }
  ...
  /** 数値入力制限があるか */
  hasNumberValidationRules() {
    // すべてのNumberValidationRuleを取得
    const allNumberValidationRules = this.getPages().flatMap(page =>
      page.getQuestions().flatMap(question =>
        question.getNumberValidationRuleMap().keySeq(),
      ),
    );

    return allNumberValidationRules.size > 0;
  }

  /** フリーモードのページがあるか */
  hasFreeModePage() {
    return this.getPages().filter(page => page.isFreeMode()).size > 0;
  }

  /** テーブルのカスタマイズを行う設問があるか */
  hasMatrixEnabledQuestion() {
    return !!this.getPages().find(page => page.hasMatrixHtmlEnabledQuestion());
  }
  ...
  isValidPositionOfCompleteFinisher() {
    ...
    if (!this.findNode(nextNodeId) || !this.findNode(nextNodeId).isFinisher()) return false;
    const finisher = this.findFinisherFromNode(nextNodeId); // Pageの次のノードのFinisherを取得
    return finisher && finisher.isComplete();
  }
  ...
  /**
   * OutputDefinitionが異なるかどうかをチェックする
   */
  isOutputDefinitionEqual(otherSurvey) {
    return this.getAllOutputDefinitions().map(od => od.getId()).join('') === otherSurvey.getAllOutputDefinitions().map(od => od.getId()).join('');
  }
   ...
   */
  clean() {
    return this.update('pages', pages => pages.map(page => page.clean(this)));
  }

  /**
   * 旧データから新データへのマイグレーションを行う
   */
  migrate(options) {
    let survey = migrateNumberValidation(this);
    survey = migrateScheduleSubItems(survey);
    survey = migrateCssUrls(survey, options);
    survey = migratePersonalInfoQuestion(survey);
    return survey;
  }

  /** CSS URL の設定があればtrue、無ければfalseを返す */
  hasCssUrls() {
    const runtimeUrls = this.getCssRuntimeUrls();
    const previewUrls = this.getCssPreviewUrls();
    const detailUrls = this.getCssDetailUrls();
    return (runtimeUrls && runtimeUrls.size >= 1) || (previewUrls && previewUrls.size >= 1) || (detailUrls && detailUrls.size >= 1);
  }
}
