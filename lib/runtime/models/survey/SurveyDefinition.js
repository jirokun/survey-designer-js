...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  /** Replacerを生成して返す */
  createReplacer(answers = {}) {
    if (this.replacerCache) return this.replacerCache;
  ...
  getAllOutputDefinitions() {
    return this.getPages().flatMap((page) => {
      ...
      return page.getQuestions().flatMap(question => question.getOutputDefinitions(pageNo, this.calcQuestionNo(page.getId(), question.getId())))
        .concat(page.getLogicalVariableOutputDefinitions());
    }).toList();
  }
  ...
  getQuestionNoMap() {
    ...
    this.getPages().forEach((page) => {
      ...
      page.getLogicalVariableOutputDefinitions().forEach((od) => { questionNoMap[od.getOutputNo()] = od.getName(); });
    });
  ...
  findPrecedingOutputDefinition(nodeId, includeCurrentPage = true) {
    ...
    return List(precedingPageNodeIds).flatMap((nodeId2) => {
      ...
      // currentPageならlogicalVariableを追加しない
      if (nodeId !== nodeId2) return outputDefinitions.concat(page.getLogicalVariableOutputDefinitions());
