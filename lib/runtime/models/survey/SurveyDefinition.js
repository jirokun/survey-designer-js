...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  findNextPageIdFromRefId(refId) {
    const nextNodeId = this.findNodeFromRefId(refId).getNextNodeId();
    if (nextNodeId === null) {
      return null;
    }
  ...
  updateAllJavaScript(idAndJs,savedAt) {
    ...
    idAndJs.entrySeq().forEach(function (e) {
      const index = newSurvey.findPageIndex(e[0]);
      newSurvey = newSurvey.updateIn(['pages', index], page => page.updatePageAttribute('javaScriptCode', e[1]));
    });
    return newSurvey;
  }
