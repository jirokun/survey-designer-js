...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  getCompleteFinisherNodes() {
    return this.getFinisherNodes().filter((node) => {
  ...
  validate() {
    ...
    // 最後のnodeはfinisherである
    if (!this.getNodes().last().isFinisher()) errors = errors.push('最後のページが終了ページではありません');
    // 最後のページの次はCOMPLETEのFinisherである
    if (!this.isValidPositionOfCompleteFinisher()) errors = errors.push('最後のページの次は「COMPLETEの終了ページ」にして下さい');
    // COMPLETEのFinisher以降にページが存在しない
    if (!this.isValidCompleteFinisher()) errors = errors.push('「COMPLETEの終了ページ」以降にページが存在します');
  ...
  /** 最後のページの次がCOMPLETEのFinisherになっているかチェック */
  isValidPositionOfCompleteFinisher() {
    const nextNodeId = this.getNodes().filter(node => node.isPage()).last().getNextNodeId();
  ...
  /**
   * 不正なCOMPLETEのFinisherが存在しないかチェック
   *
   * COMPLETEページ以降にページが存在しなければ true、すれば false
   */
  isValidCompleteFinisher() {
    return this.getCompleteFinisherNodes().filter((node) => {
      // Complete Finisher以降にページが存在するか
      const nodeIds = this.findFollowingPageNodeIds(node.getId());
