...
export default class SurveyDefinition extends SurveyDefinitionRecord {
  ...
  validate() {
    ...
    if (!this.isValidPositionOfCompleteFinisher()) errors = errors.push('全員がスクリーンアウトしてしまう事故防止のために、一番最初の終了ページは「COMPLETE」にして下さい');
    // Finisher以降にページが存在しない
    if (!this.isValidFinisher()) errors = errors.push('終了ページより後ろに「ページ」を設置しないで下さい');
