...
export default class PageDefinition extends PageDefinitionRecord {
  ...
  setFreeMode(freeMode) {
    return this.set('freeMode', freeMode);
  }
    ...
    text.split('\n').map(str => S(str).trim()).forEach((itemStr) => {
      ...
      items = items.push(
        new ItemDefinition({
          _id: cuid(),
          index: items.size,
          devId: surveyDevIdGeneratorInstance.generateForItem(questionId),
          value: '',
          label: itemStr.s,
