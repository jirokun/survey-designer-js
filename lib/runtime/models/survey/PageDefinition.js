...
export default class PageDefinition extends PageDefinitionRecord {
  ...
  getLogicalVariableOutputDefinitions() {
  ...
  addLogicalVariable(survey) {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      if (this.isUniqueLogicalVariableName(logicalVariable.getId(), name)) break;
    }
    return this.update('logicalVariables', logicalVariables => logicalVariables.push(logicalVariable.set('variableName', name)));
  }
  ...
  updateItemAttribute(questionId, itemId, attributeName, value, replacer) {
    ...
    return this.setIn(['questions', questionIndex, 'items', itemIndex, attributeName], replacer.outputNo2Name(value));
  }
  ...
  updateSubItemAttribute(questionId, itemId, attributeName, value, replaceUtil) {
    ...
    return this.setIn(['questions', questionIndex, 'subItems', itemIndex, attributeName], replaceUtil.outputNo2Name(value));
  }
  ...
  updateQuestionAttribute(questionId, attributeName, value1, value2, replacer) {
    ...
    if (attributeName === 'title') {
      const newState = this.setIn(['questions', questionIndex, attributeName], replacer.outputNo2Name(value1))
        .setIn(['questions', questionIndex, 'plainTitle'], replacer.outputNo2Name(value2));
      return newState;
    }
    if (attributeName === 'items') {
      throw new Error('updateQuestionAttributeではitemsを更新できません');
    }
    return this.setIn(['questions', questionIndex, attributeName], replacer.outputNo2Name(value1));
  }
