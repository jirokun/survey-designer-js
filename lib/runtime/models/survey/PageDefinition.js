...
export default class PageDefinition extends PageDefinitionRecord {
  static create() {
    const id = surveyIdGeneratorInstance.generatePageId();
  ...
  addItem(questionId, index) {
    ...
    const newQuestion = question.set('items', question.getItems().insert(index, ItemDefinition.create(itemId, index)));
  ...
  bulkAddItemsInner(questionId, text, prop) {
    ...
    text.split('\n').map(str => S(str).trim()).forEach((itemStr) => {
      ...
      items = items.push(
        new ItemDefinition({
          _id: surveyIdGeneratorInstance.generateItemId(),
          index: items.size,
          value: '',
          label: itemStr.s,
  ...
  addSubItem(questionId, index) {
    ...
    const question = this.findQuestion(questionId);
    const itemId = surveyIdGeneratorInstance.generateItemId();
    const newQuestion = question.set('subItems', question.getSubItems().insert(index, ItemDefinition.create(itemId, index)));
  ...
  addQuestion(questionClassName, pageId, index) {
    const modelMap = findQuestionDefinitionMap(questionClassName, { pageId });
    const question = modelMap.definitionClass.create(modelMap.options);
