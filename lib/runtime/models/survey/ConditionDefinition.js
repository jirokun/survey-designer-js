...
export default class ConditionDefinition extends ConditionDefinitionRecord {
  ...
  validate(survey) {
    ...
    if (nodes.findIndex(node => node.getId() === this.getNextNodeId()) === -1) errors = errors.push('分岐設定の遷移先が存在しません');
    return errors.concat(this.getChildConditions().flatMap(cc => cc.validate(survey)));
  }
}
