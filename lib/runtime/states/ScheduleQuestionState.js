...
export default class ScheduleQuestionState extends ScheduleQuestionStateRecord {
  constructor(itemSize) {
    const checkState = Repeat(List([false, false, false]), itemSize) // itemsの数だけ[fasle, false, false]を作成する
      .toList()
      .push(List([false])); // 「上記のいずれも都合がつかない」分のbooleanを追加
    super({ checkState });
  }
  getCheckState() {
  ...
  /** 「上記のいずれも都合がつかない」がチェックされているかどうかを返す */
  isOtherChecked() {
    return this.get('checkState').last().first();
  }
}
