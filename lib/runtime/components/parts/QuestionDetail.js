...
export default class QuestionDetail extends Component {
  ...
  createOptions() {
    const { question, random, subItemsRandom, matrixReverse, matrixHtml, optional } = this.props;

    if (!random && !subItemsRandom && !matrixReverse && !matrixHtml && !optional) return null;
          ...
          <tbody>
            {
              optional ?
                <tr>
                  <th>{optional === true ? '任意入力' : optional}</th>
                  <td>{question.isOptional() ? '◯' : null}</td>
                </tr> : null
            }
            {
              random ?
