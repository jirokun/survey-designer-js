...
export const InterviewRow = (props) => {
  ...
  const workContent = (!props.mobileTel.isDisable() && <label className="personalInfo-label">
    <input
...
export const ScheduleRow = (props) => {
  ...
  const mobileContent = (!props.mobileTel.isDisable() && <label className="personalInfo-label">
    ...
    />
    <span id={`${props.question.getOutputName(FIELDS.scheduleContactMobileTel)}__text`} className={classNames('select-tool check-not-text', { disabled: !props.mobileTel.isRequire() })}>携帯電話</span>
  </label>);
  ...
  const homeContent = (!props.homeTel.isDisable() && <label className="personalInfo-label">
    <input
      ...
      data-parsley-multiple={scheduleContactCheckboxGroup}
    />
    <span id={`${props.question.getOutputName(FIELDS.scheduleContactHomeTel)}__text`} className={classNames('select-tool check-not-text', { disabled: !props.homeTel.isRequire() })}>TEL（自宅）</span>
  </label>);
  ...
  const workContent = (!props.workTel.isDisable() && <label className="personalInfo-label">
    <input
      ...
      data-parsley-multiple={scheduleContactCheckboxGroup}
    />
    <span id={`${props.question.getOutputName(FIELDS.scheduleContactWorkTel)}__text`} className={classNames('select-tool check-not-text', { disabled: !props.workTel.isRequire() })}>TEL（勤務先）</span>
  </label>);
  ...
  const emailContent = (!props.email.isDisable() && <label className="personalInfo-label">
    <input
      ...
      data-parsley-multiple={scheduleContactCheckboxGroup}
    />
    <span id={`${props.question.getOutputName(FIELDS.scheduleContactEmail)}__text`} className={classNames('select-tool check-not-text', { disabled: !props.email.isRequire() })}>Eメール</span>
  </label>);
