...
export default class MultiNumberQuestion extends TransformQuestion {
  ...
  createItem(item, isTotal = false) {
    ...
    const totalHiddenId = `${question.getOutputName(true)}_hidden`;
    return (
      <div ref={(el) => { this.rootEl = el; }} key={`${question.getId()}_${item.getIndex()}`} className={classNames('multiNumberLine', { multiNumberTotal: isTotal })}>
        <div className="multiNumberLabel">
          <label dangerouslySetInnerHTML={{ __html: replaceUtil.name2Value(item.getLabel()) }} />
        </div>
        <div className="multiNumberAssociated mini">
          <div className="multiNumberInput">
            <input
              ...
              name={name}
              data-key="value"
              className={classNames({ disabled: isTotal })}
              readOnly={isTotal}
              data-response-multiple
              data-response-item-index={isTotal ? null : item.getIndex()}
              data-parsley-required={!isTotal}
              data-parsley-type="digits"
              data-parsley-min={isTotal || S(question.getMin()).isEmpty() ? null : replaceUtil.name2Value(question.getMin())}
              data-parsley-max={isTotal || S(question.getMax()).isEmpty() ? null : replaceUtil.name2Value(question.getMax())}
              data-parsley-equalto={isTotal && !S(totalEqualTo).isEmpty() ? `#${totalHiddenId}` : null}
              data-parsley-equalto-message={`合計が${replaceUtil.name2Value(totalEqualTo)}になるように入力してください`}
            />
            { isTotal ? <input type="hidden" id={totalHiddenId} value={replaceUtil.name2Value(totalEqualTo)} /> : null }
