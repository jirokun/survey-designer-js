...
class MultiNumberQuestion extends TransformQuestion {
  ...
  createQuestionDetail() {
    const { question, options, replacer } = this.props;
    if (!options.isShowDetail()) return null;
    ...
    return (
      <ul className="question-detail">
        ...
        {!S(min).isEmpty() && !replacer.isIncludeName(min) ? <li>各項目の入力値制限 最小値: {min}</li> : null}
        {!S(min).isEmpty() && replacer.isIncludeName(min) ? <li>各項目の入力値制限 最小値: 次の設問の入力と同値({replacer.findReferenceOutputDefinitions(min)[0].getLabelForDetail()})</li> : null}
        {!S(max).isEmpty() && !replacer.isIncludeName(max) ? <li>各項目の入力値制限 最大値: {max}</li> : null}
        {!S(max).isEmpty() && replacer.isIncludeName(max) ? <li>各項目の入力値制限 最大値: 次の設問の入力と同値({replacer.findReferenceOutputDefinitions(max)[0].getLabelForDetail()})</li> : null}
        {question.isShowTotal() && !S(totalEqualTo).isEmpty() && !replacer.isIncludeName(totalEqualTo) ? <li>合計の入力値制限: 次の設問の入力と同値({totalEqualTo})</li> : null}
        {question.isShowTotal() && !S(totalEqualTo).isEmpty() && replacer.isIncludeName(totalEqualTo) ? <li>合計の入力値制限: 次の値と同値({replacer.findReferenceOutputDefinitions(totalEqualTo)[0].getLabelForDetail()})</li> : null}
      </ul>
    );
  }
