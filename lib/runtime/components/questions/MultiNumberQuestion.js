...
class MultiNumberQuestion extends TransformQuestion {
  ...
  setTotalValue() {
    ...
    const idList = this.state.model.getTransformedItems().map(item => question.getOutputName(null, item.getIndex())).toList();
    ...
    setTimeout(() => {
      ...
      const totalId = question.getOutputName('total');
  ...
  createAdditionalInput(item, question, errorContainerId) {
    ...
    const name = question.getOutputName('additional', item.getIndex());
  ...
  createItem(item, isTotal = false) {
    ...
    const name = question.getOutputName(null, item.getIndex());
