  ...
  createQuestionDetail() {
    ...
    return (
      <ul className="question-detail">
        ...
        {!S(min).isEmpty() && !replacer.isIncludeName(min) ?
          <li>各項目の最小値: <span className="fixed-value">{min}</span></li> : null}
        {!S(min).isEmpty() && replacer.isIncludeName(min) ?
          <li>各項目の最小値: <span className="answer-value">{replacer.findReferenceOutputDefinitions(min)[0].getLabelForDetail()}</span></li> : null}
        {!S(max).isEmpty() && !replacer.isIncludeName(max) ?
          <li>各項目の最大値: <span className="fixed-value">{max}</span></li> : null}
        {!S(max).isEmpty() && replacer.isIncludeName(max) ?
          <li>各項目の最大値: <span className="answer-value">{replacer.findReferenceOutputDefinitions(max)[0].getLabelForDetail()}</span></li> : null}
        {question.isShowTotal() && !S(totalEqualTo).isEmpty() && !replacer.isIncludeName(totalEqualTo) ?
          <li>合計が次の値になること: <span className="fixed-value">{totalEqualTo}</span></li> : null}
        {question.isShowTotal() && !S(totalEqualTo).isEmpty() && replacer.isIncludeName(totalEqualTo) ?
          <li>合計が次の値になること: <span className="answer-value">{replacer.findReferenceOutputDefinitions(totalEqualTo)[0].getLabelForDetail()}</span></li> : null}
      </ul>
    );
  }
