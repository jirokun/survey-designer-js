...
class MultiNumberQuestion extends TransformQuestion {
  ...
  setTotalValue() {
    ...
    const idList = this.state.model.getTransformedItems().map(item => question.getOutputName(false, item.getId())).toList();
  ...
  createItem(item, isTotal = false) {
    ...
    const name = question.getOutputName(isTotal, item.getId());
  ...
  createTotalItem() {
    ...
    const id = surveyIdGeneratorInstance.generateItemId();
    return this.createItem(new ItemDefinition({ _id: id, label: '合計' }), true);
  }
