...
class MatrixQuestion extends TransformQuestion {
  ...
  getAdditionalDevId(question, itemDevId, subItemDevId) {
    if (question.isMatrixReverse()) {
      return question.getOutputDevId(subItemDevId, itemDevId, true);
    }
    return question.getOutputDevId(itemDevId, subItemDevId, true);
  }

  getOutputDevId(question, itemDevId, subItemDevId) {
    if (question.isMatrixReverse()) {
      return question.getOutputDevId(subItemDevId, itemDevId, false);
    }
    return question.getOutputDevId(itemDevId, subItemDevId, false);
  }
  ...
  createAdditionalInputElement(question, item, subItem) {
    ...
    const devId = this.getAdditionalDevId(question, item.getDevId(), subItem.getDevId());
  ...
  createInputElement(tableId, rowId, question, item, subItem) {
    ...
    const devId = this.getOutputDevId(question, item.getDevId(), subItem.getDevId());
              ...
              return (
                <td key={colId} className={columnClassName}>
                  ...
                  <input
                    ...
                    data-dev-id={`${item.getDevId()}_total_column`}
                  />
                </td>
              );
            })
      ...
      sumEl = (
        <td className={classNames({ 'detail-shown': showValidation })}>
          ...
          <input
            ...
            data-parsley-positive-integer
            data-parsley-equal={!S(totalEqualTo).isEmpty() ? replacer.id2Value(totalEqualTo) : null}
            data-dev-id={`${item.getDevId()}_total_row`}
          />
        </td>
      );
    }
