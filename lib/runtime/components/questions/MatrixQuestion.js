...
export default class MatrixQuestion extends TransformQuestion {
  static createRow(question, item, itemIndex, replacer) {
            ...
            <thead>
              <tr>
                ...
                {question.getTransformedSubItems().map(subItem =>
                  <td key={subItem.getId()} dangerouslySetInnerHTML={{ __html: replacer.name2Value(subItem.getLabel()) }} />)}
              </tr>
            </thead>
            <tbody>
              {question.getTransformedItems().map((item, itemIndex) => MatrixQuestion.createRow(question, item, itemIndex, replacer))}
            </tbody>
          </table>
        </div>
      </div>
