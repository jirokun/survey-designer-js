...
class MatrixQuestion extends TransformQuestion {
  ...
  componentDidMount() {
    ...
    // 縦書きのヘッダー項目を取得する
    var elements = document.getElementsByClassName( "vertical-writing" ) ;
    for ( var i = 0,l = elements.length; l > i; i++ ) {
      var element = elements[i] ;
      var rect = element.getBoundingClientRect();
      
      // 親TD幅　<　div幅だったら、TD幅にdiv幅を設定する
      if (element.parentNode.clientWidth < rect.width) {
        element.parentNode.style.width = rect.width + 'px';
                  ...
                  return (
                    <td key={subItem.getId()} id={subItem.getId()} className={columnClassName} data-col-order={subItem.getIndex() + 1}>
                      {this.createItemDetail(subItem)}
                      {
                        ...
                        <div className={innerColumnClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Span(subItem.getLabel()) }} />
                      }
                    </td>
                  );
                })}
