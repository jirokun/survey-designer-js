  ...
  render() {
    ...
    return (
      <div className={this.constructor.name}>
        {S(title).isEmpty() ? null : <h2 className="question-title" data-dev-id-label={question.getDevId()} dangerouslySetInnerHTML={{ __html: replacer.id2Value(title) }} />}
                ...
                {subItems.map((subItem) => {
                  ...
                  return (
                    <td key={subItem.getId()} id={subItem.getId()} className={columnClassName} data-col-order={subItem.getIndex() + 1}>
                      ...
                      {
                        question.isMatrixReverse() ? <div id={this.getLabelId(subItem)} className={innerColumnClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Value(subItem.getLabel()) }} /> :
                        <div className={innerColumnClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Value(subItem.getLabel()) }} />
                      }
                    </td>
                  );
                })}
