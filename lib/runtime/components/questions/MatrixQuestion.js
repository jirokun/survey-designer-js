...
export default class MatrixQuestion extends Component {
    ...
    return (
      <tr key={rowId} id={rowId} data-row-order={item.getIndex() + 1}>
        <td className={classNames({ 'detail-shown': question.isRandom() && item.isRandomFixed() })}>
          <span id={question.getLabelId(item)} dangerouslySetInnerHTML={{ __html: replacer.id2Span(item.getLabel()) }} />
  ...
  static createQuestionElement(survey, question, options) {
    ...
    if (matrixHtml !== null) return <div className="question" dangerouslySetInnerHTML={{ __html: replacer.id2Span(matrixHtml) }} />;
              ...
              {subItems.map((subItem) => {
                ...
                return (
                  <td key={subItem.getId()} id={subItem.getId()} className={columnClassName} data-col-order={subItem.getIndex() + 1}>
                    ...
                    <div><div id={question.getLabelId(subItem)} className={innerColumnClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Span(convertVerticalWriting(innerColumnClassName, subItem.getLabel())) }} /></div>
