...
class MatrixQuestion extends TransformQuestion {
  ...
  createInputElement(tableId, rowId, question, item, subItem) {
    ...
    const outputNo = outputDefinition.getOutputNo();
    switch (matrixType) {
      case 'radio': {
              ...
              return (
                <td key={colId} className={classNames({ 'detail-shown': showValidation })}>
                  ...
                  <input
                    ...
                    size="4"
                    defaultValue="0"
                    data-output-no={survey.findOutputDefinitionFromName(totalName).getOutputNo()}
                    data-parsley-required
                    data-parsley-type="digits"
  ...
  createRow(tableId, item, itemIndex, subItems, replacer) {
    const { survey, options, question } = this.props;
