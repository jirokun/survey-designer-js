...
class MatrixQuestion extends TransformQuestion {
  ...
  getAdditionalOutputName(question, itemId, subItemId) {
    if (question.isMatrixReverse()) {
      return question.getAdditionalOutputName(subItemId, itemId);
    }
    return question.getAdditionalOutputName(itemId, subItemId);
  }

  getOutputName(question, itemId, subItemId) {
    if (question.isMatrixReverse()) {
      return question.getOutputName(subItemId, itemId);
    }
    return question.getOutputName(itemId, subItemId);
  }
  ...
  createAdditionalInputElement(question, item, subItem) {
    ...
    const name = this.getAdditionalOutputName(question, item.getId(), subItem.getId());
  ...
  createInputElement(tableId, rowId, question, item, subItem) {
    const { survey } = this.props;
    const matrixType = question.getMatrixType();
    const name = this.getOutputName(question, item.getId(), subItem.getId());
    ...
    if (question.getMatrixType() === 'number' && question.isMatrixSumCols()) {
      return (
        <tr data-row-order="total">
          ...
          {
            items.map((item) => {
              const totalName = question.getOutputTotalColName(item.getId());
  ...
  createRow(tableId, item, itemIndex, subItems, replacer) {
    ...
    const totalName = question.getOutputTotalRowName(item.getId());
    let sumEl = null;
