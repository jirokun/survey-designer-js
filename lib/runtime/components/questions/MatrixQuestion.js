...
export default class MatrixQuestion extends TransformQuestion {
  ...
  static createAdditionalInputElement(question, item, subItem) {
    ...
    const targetItem = question.isMatrixReverse() ? subItem : item;
    ...
    return (
      <span>
        <input type={targetItem.getAdditionalInputType()} name={name} />
        {targetItem.getUnit()}
      </span>
    );
  }
  ...
  static createRow(tableId, question, item, itemIndex, subItems, replacer) {
    ...
    return (
      <tr key={rowId} id={rowId}>
        ...
        {question.isMatrixSumRows() ? <td><span className="sdj-sum">0</span><input name={totalName} className="sdj-sum-hidden" type="hidden" /></td> : null}
      </tr>
    );
  }

  static createSumCol(question, items) {
        ...
        <tr>
          <td>合計</td>
          {
            items.map((item) => {
              ...
              return <td><span className="sdj-sum">0</span><input name={totalName} className="sdj-sum-hidden" type="hidden" /></td>;
            })
          }
        </tr>
      );
    ...
    return (
      <div ref={(el) => { this.rootEl = el; }} className={this.constructor.name}>
        ...
        <div className="question">
          <table
            className={
              classNames('matrix group question-form-grid-table',
                {
                  'sdj-sum-cols': question.isMatrixSumCols(),
          ...
          >
            ...
            <tbody>
              ...
              {MatrixQuestion.createSumCol(question, items)}
            </tbody>
          </table>
        </div>
      </div>
