...
export default class MatrixQuestion extends TransformQuestion {
  ...
  static createRow(tableId, question, item, itemIndex, subItems, replacer) {
    ...
    if (question.isMatrixSumRows()) {
      const totalHiddenId = `${item.getId()}_hidden`;
      ...
      sumEl = (
        <td>
          <input
            ...
            data-parsley-equalto={!S(totalEqualTo).isEmpty() ? `#${totalHiddenId}` : null}
            data-parsley-equalto-message={`合計が${replacer.name2Value(totalEqualTo)}になるように入力してください`}
          />
          <input type="hidden" id={totalHiddenId} value={replacer.name2Value(totalEqualTo)} />
        </td>
      );
    }
    return (
        ...
        <tr>
          <td>合計</td>
          {
            items.map((item) => {
              ...
              return <td key={colId}><span className="sdj-sum">0</span><input name={totalName} className="sdj-sum-hidden" type="hidden" /></td>;
            })
          }
        </tr>
      );
