              ...
              <tr>
                ...
                {subItems.map((subItem) => {
                  const columnClassName = classNames({
                    ...
                    [subItem.calcVisibilityClassName(survey, runtime.getAnswers())]: !options.isVisibilityConditionDisabled(),
                    'col-vertical-writing':question.isMatrixTategakiCols(),
                  });
                  const divClassName = classNames({
                    'vertical-writing':question.isMatrixTategakiCols(),
                  });
                  return (
                    <td key={subItem.getId()} id={subItem.getId()} className={columnClassName} data-col-order={subItem.getIndex() + 1}>
                      ...
                      {
                        question.isMatrixReverse() ? <div id={this.getLabelId(subItem)} className={divClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Value(subItem.getLabel()) }} /> :
                        <div className={divClassName} dangerouslySetInnerHTML={{ __html: replacer.id2Value(subItem.getLabel()) }} />
                      }
                    </td>
                  );
                })}
