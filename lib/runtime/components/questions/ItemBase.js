...
class Item extends Component {
  /** addtionalInputを描画する */
  renderAdditionalInput(itemState, item, question) {
  ...
  render() {
    const { containerId, itemState, item, question, handleItemCheckedChange } = this.props;
    const label = item.getLabel();
    const unit = item.getUnit();
    const answers = {};
    const disabled = !!itemState.get('disabled');
    const className = `question-form-list ${disabled ? 'disabled' : ''}`;
    const inputType = question.getDataType().toLowerCase();
    ...
    return (
      <li className={className}>
        <label className="question-form-list-label">
          ...
          {this.renderAdditionalInput(itemState, item, question)}
