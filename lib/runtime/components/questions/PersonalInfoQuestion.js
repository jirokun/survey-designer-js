...
export default class PersonalInfoQuestion extends Component {
  ...
  generateRow(survey, question, item, homeTelEmpty) {
    switch (item.getRowType()) {
      ...
      case 'InterviewRow':
        return <InterviewRow key={item.getId()} survey={survey} question={question} item={item} homeTelEmpty={homeTelEmpty} />;
      case 'MobileTelRow':
      ...
      case 'ScheduleRow':
        return <ScheduleRow key={item.getId()} survey={survey} question={question} item={item} />;
      case 'SexRow':
