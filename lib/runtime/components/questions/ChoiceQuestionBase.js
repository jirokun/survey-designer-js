  ...
  createQuestionDetail() {
    ...
    return (
      <ul className="question-detail">
        ...
        {question.getMinCheckCount() > 0 ? <li>最小チェック数: {question.getMinCheckCount()}</li> : null}
        {question.getMaxCheckCount() > 0 ? <li>最大チェック数: {question.getMaxCheckCount()}</li> : null}
      </ul>
    );
  }
...
class Item extends Component {
  ...
  render() {
    const { replacer, containerId, itemState, item, question, inputType, handleItemCheckedChange } = this.props;
