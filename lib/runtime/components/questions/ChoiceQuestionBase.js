...
export default class ChoiceQuestionBase extends TransformQuestion {
  ...
  createItems(containerId) {
    const { survey, page, replacer, question, options } = this.props;
    const { model } = this.state;
...
class Item extends Component {
  ...
  createAdditionalInput(itemState, item, question, errorContainerId) {
    ...
    const outputDefinition = survey.findOutputDefinitionFromName(name);
    ...
    const outputNo = !outputDefinition ? null : outputDefinition.getOutputNo();
  ...
  render() {
    const { survey, options, replacer, containerId, itemState, item, question, inputType, handleItemCheckedChange } = this.props;
