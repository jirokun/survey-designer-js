...
export default class PersonalInfoJS {
  ...
  setHomeTelEvent(question) {
    ...
    // 自宅電話番号にイベントを設定
    if (this.isOptionalItem(homeTelCheckboxOutputDefinitions)) { // 任意の質問のみ対象
      // 自宅電話番号のフォーム要素
  ...
  setMobileTelEvent(question) {
    ...
    // 携帯電話番号にイベントを設定
    if (this.isOptionalItem(mobileTelCheckboxOutputDefinitions)) { // 任意の質問のみ対象
      // 携帯電話番号のフォーム要素
  ...
  setWorkTelEvent(question) {
    ...
    // 勤務先電話番号にイベントを設定
    if (this.isOptionalItem(workTelCheckboxOutputDefinitions)) { // 任意の質問のみ対象
      // 勤務先電話番号のフォーム要素
  ...
  setEmailEvent(question) {
    ...
    // Emailにイベントを設定
    if (this.isOptionalItem(emailCheckboxOutputDefinitions)) { // 任意の質問のみ対象
      // Emailのフォーム要素
