...
export default class MatrixQuestionJS {
  ...
  /** Matrixの数値設問を取得する */
  findMatrixRadioQuestions() {
    return this.page.getQuestions().filter(question => question.getDataType() === 'Matrix' && question.getMatrixType() === 'radio');
  }
  ...
  /**
   * 縦方向のユニーク制約を追加(ランキング設問)
   *
   * @param {MatrixQuestionDefinition} question 設問のインスタンス
   */
  matrixRowAndColumnUniqueRadio(question) {
    if (!question.isMatrixRowAndColumnUnique()) return;
    ...
    question.getSubItems()
      .forEach((subItem) => {
        question.getItems().forEach((item) => {
          const checkboxOutputDefinition = question.getOutputDefinitionFromItemAndSubItem(item, subItem);
          const query = createQueryForOutputDefinition(checkboxOutputDefinition);
          $el.on('change', query, this.removeSameValue.bind(this, question));
        });
      });
  }
  ...
  /** 設定したラジオボタンと同じ値を設定した別のラジオボタンを非選択にする */
  removeSameValue(question, e) {
    question.getItems().forEach((item) => {
      question.getSubItems().forEach((subItem) => {
        // ODの要素を取得
        const outputDefinition = question.getOutputDefinitionFromItemAndSubItem(item, subItem);
        const $el = $(`[name="${outputDefinition.getName()}"]:checked`);

        // 異なる列で同じ値の場合
        if (e.target.name !== $el.attr('name') && $el.val() === e.target.value && $el.prop('checked')) {
          $el.prop('checked', false); // checkedを外す
        }
      });
    });
  }
  ...
  showExclusiveItem(question) {
    ...
    (question.isMatrixReverse() ? question.getItems() : question.getSubItems())
      .filter(item => item.isExclusive())
      .forEach((item) => {
  ...
  showRandomFixed(question) {
    const targetItemsArray = [];
    if (question.isRandom()) targetItemsArray.push(question.getItems());
    if (question.isSubItemsRandom()) targetItemsArray.push(question.getSubItems());
    targetItemsArray.forEach((items) => {
      items.filter(item => item.isRandomFixed())
        .forEach((item) => {
          const labelId = question.getLabelId(item);
  ...
  optionalize(question) {
    ...
    (question.isMatrixReverse() ? question.getSubItems() : question.getItems())
      .filter(subItem => subItem.isOptional())
      .forEach((subItem) => {
        const sameItemGroupOutputDefinitions = question.getSameItemGroupOutputDefinition(subItem);
  ...
  showOptional(question) {
    ...
    (question.isMatrixReverse() ? question.getSubItems() : question.getItems())
      ...
      .forEach((item) => {
        const labelId = question.getLabelId(item);
        $(this.el).find(`#${labelId}`).after('<span class="detail-function">任意</span>');
      });
  }
  ...
  initialize() {
    ...
    // Matrixラジオボタン
    this.findMatrixRadioQuestions().forEach((question) => {
      this.matrixRowAndColumnUniqueRadio(question); // 縦方向のユニーク制約(ランキング設問)
    });
  }
  ...
  initForDetail() {
    this.survey.getPages()
      ...
      .forEach((question) => {
        ...
        this.showOptional(question);                     // 任意の表示
        MatrixQuestionJS.convertCheckboxAndRadio(question, this.el);
      });
  }
