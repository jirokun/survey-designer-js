...
export default class Matrix {
  ...
  additionalInputControl(question) {
    ...
    const query = outputDefinitionsOfAdditionalInput.map(od => createQueryForOutputDefinition(od)).join(',');
    const $table = this.findTable(question);
    $table.on('change', query, this.updateCellByAdditionalInput.bind(this, question));
  }
