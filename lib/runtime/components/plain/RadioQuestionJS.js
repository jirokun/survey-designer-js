...
export default class RadioQuestionJS extends QuestionWithItemBaseJS {
  ...
  showRandomFixed(question) {
    question
      ...
      .forEach((item) => {
        const query = `${createQueryForOutputDefinition(question.getOutputDefinition())}[value="${item.getValue()}"]`;
        $(this.el).find(query).after('<span class="detail-function">ランダム固定</span>');
      });
  }
  ...
  optionalize(question) {
    ...
    question
      ...
      .forEach((item) => {
        const query = `${createQueryForOutputDefinition(question.getOutputDefinition())}[value="${item.getValue()}"]`;
        $(this.el).find(query).removeAttr('data-parsley-required');
      });
  }
  ...
  initForDetail() {
    this.survey.getPages()
      ...
      .filter(question => question.getDataType() === 'Radio')
      .forEach((question) => {
