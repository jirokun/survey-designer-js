    ...
    $(this.el).on('click', 'img[data-popup-url]', (e) => {
      ...
      gallery.listen('imageLoadComplete', (index, item) => {
        ...
        img.onload = () => {
          ...
          item.w = img.width;
          item.h = img.height;
          // タイミング的にすでにcloseされている可能性がある。そのためopenされていることを確認してupdateSizeする
          const pswpIsOpened = $(gallery.container).parents('.pswp').hasClass('pswp--open');
          if (pswpIsOpened) gallery.updateSize(true);
        };
        img.src = item.src;
      });
