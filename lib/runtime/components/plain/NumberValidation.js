...
export default class NumberValidation {
  ...
  updateTargetElementParsleyAttribute(targetName, numberValidation, referenceEl) {
    const value = zenkakuNum2Hankaku(referenceEl.value);
   ...
   */
  updateTargetElementParsleyAttributeForLogicalVariable(targetName, numberValidation, logicalVariable, withoutValidation = false) {
    ...
    const $target = $(`*[name="${targetName}"]`);
    const parsleyField = $target.attr(OPERATOR_MAP[operator], value).parsley();
    // withoutValidationが明示的に指定されている、または対象でなければvalidateしない
    if (withoutValidation === true || $target.is(EXCLUDED_OPTION)) return;
    parsleyField.validate();
  }
  ...
  attachEventForReferenceElement(referenceOutputDefinition, targetName, numberValidation) {
    ...
    if (!referenceEl) return; // フリーモードなどで消されているとelが存在しない
    const mutationObserver = new MutationObserver(() => {
      this.updateTargetElementParsleyAttribute(targetName, numberValidation, referenceEl);
    });
    mutationObserver.observe(referenceEl, { attributes: true, attributeFilter: ['disabled', 'value'] });
  ...
  attachEventForLogicalVariable(logicalVariableOutputDefinition, targetName, numberValidation) {
    ...
    // 初期化時に一度実行する
    this.updateTargetElementParsleyAttributeForLogicalVariable(targetName, numberValidation, logicalVariable, true);
    ...
    logicalVariable
      ...
      .forEach((od) => { // ロジック変数が参照している同じページ内にある要素が出力するOutputDefinition毎に処理
        ...
        // 値が変更されたときのイベントを割り当て
        $(this.el).on('change', query, this.updateTargetElementParsleyAttributeForLogicalVariable.bind(this, targetName, numberValidation, logicalVariable));
