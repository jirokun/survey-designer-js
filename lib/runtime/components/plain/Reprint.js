...
export default class Reprint {
  ...
  attachEventForLogicalVariable(logicalVariable, $target) {
    logicalVariable
      .getOperands()
    ...
    $(this.el).find('.answer-value[data-reprint]').each((i, target) => {
      ...
      if (this.isOutputDefinitionsInCurrentPage(outputDefinitionId)) {
        ...
        if (outputDefinition.isLogicalVariableOutputDefinition()) {
          ...
          const logicalVariable = this.page.getLogicalVariables().find(lv => lv.getId() === outputDefinition.getId());
          // ロジック変数の場合、参照先の要素いずれかが変更されたときにバリデーションの属性を更新する処理を入れる
          this.attachEventForLogicalVariable(logicalVariable, $target);
          this.updateTargetElementForLogicalVariable($target, logicalVariable);
        } else {
          const referenceName = outputDefinition.getName();
          const query = `*[name="${referenceName}"]`;
          const referenceEl = $(this.el).find(query)[0];
          if (referenceEl) { // フリーモードではreferenceElが無いことがある
            // 回答値の場合、参照先の値が書き換わったときにバリデーションの属性を更新する処理を入れる
            this.attachEventForReferenceElement(outputDefinition, $target);
            this.updateTargetElementForCurrentPageReference($target, referenceEl, outputDefinition);
          }
        }
      } else {
        // 現在ページよりも前のページに有る回答値
