...
export default class Reprint {
  ...
  normalizeValue(el, outputDefinition) {
    ...
    if (outputType === 'checkbox') { // checkboxの場合
      return el.checked ? outputDefinition.getLabel() : '';
    } else if (outputDefinition.isOutputTypeSingleChoice()) { // radio, selectの場合
      ...
      const hankakuNumberStr = el.value.replace(/[０-９]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 65248));
  ...
  attachEventForReferenceElement(referenceOutputDefinition, $target) {
    ...
    const mutationObserver = new MutationObserver((mutationRecords) => {
      ...
      this.updateTargetElementForCurrentPageReference($target, referenceOutputDefinition, referenceEl);
    });
    ...
    logicalVariable
      ...
      .forEach((od) => { // ロジック変数が参照している同じページ内にある要素が出力するOutputDefinition毎に処理
        ...
        const mutationObserver = new MutationObserver((mutationRecords) => {
          // disabledが変更されていなければ何もしない
          if (!mutationRecords.find(record => record.attributeName === 'disabled')) return;
          this.updateTargetElementForLogicalVariable(targetName, logicalVariable, referenceEl);
        });
        mutationObserver.observe(referenceEl, { attributes: true });
        // 後処理できるようにインスタンスを保存しておく
        this.observers.push(mutationObserver);
