...
class Finisher extends Component {
  ...
  createAnswerDownloadForm() {
    ...
    return (
      <form action={options.getPreviewDownloadUrl()} method="POST" className="notice">
        <input type="hidden" name="survey" value={survey.toJson()} />
        <input type="hidden" name="output_definitions" value={survey.getOutputDefinitionsJson()} />
  ...
  createNotice() {
    ...
    return (
      <div dangerouslySetInnerHTML={{ __html: replacer.id2Span(finisher.getHtml()) }} />
    );
  }

  render() {
    const { runtime, finisher } = this.props;
    const postAnswerStatus = runtime.getPostAnswerStatus();
    return (
      <div className="finisher" id={finisher.getId()}>
        ...
        <div className={classNames('formButtons', { hidden: postAnswerStatus !== ANSWER_POSTED_SUCCESS })}>
