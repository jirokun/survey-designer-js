...
class Finisher extends Component {
  componentDidMount() {
    const { updatePostStatus, postAnswer } = this.props;
    if (!postAnswer) {
  ...
  render() {
    ...
    if (postAnswerStatus === C.ANSWER_POSTED_FAILED) {
      return <div className="notice">回答の登録に失敗しました。<br/>
...
const actionsToProps = dispatch => ({
  ...
  postAnswer: (postUrl, answers, isComplete, extraPostParameters) => Actions.postAnswer(dispatch, postUrl, answers, isComplete, extraPostParameters),
});
