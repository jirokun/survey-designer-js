...
import { showOutputNo, findVisibleFormElement } from '../../utils';
...
class Page extends Component {
  ...
  hideQuestionIfNoInputElements() {
    $(this.pageEl).find('.questionBox').each((index, questionBoxEl) => {
      const $visibleElements = findVisibleFormElement(questionBoxEl);
  ...
  skipPageIfNoInputElements() {
    const $visibleElements = findVisibleFormElement(this.pageEl);
    if ($visibleElements.length === 0) {
