...
class Page extends Component {
  ...
  componentDidMount() {
    ...
    this.parsley();
    if (options.isShowOutputNo()) Utils.showOutputNo(this.pageEl);
  ...
  handleClickNext() {
    ...
    const pageAnswers = this.getAnswers();
    const mergedAnswers = Object.assign({}, answers, pageAnswers);
    this.surveyManager.answers = mergedAnswers; // getAnswer等で使用するため設定
  ...
  skipPageIfNoInputElements() {
    const $visibleElements = $(this.pageEl).find('input:visible,select:visible,textarea:visible');
  ...
  evaluateJavaScript() {
    ...
    try {
      new Function('$', 'document', 'pm', 'sm', javaScriptCode)($, ownerDoc, this.pageManager, surveyManager);
      const answers = runtime.getAnswers().toJS();
      this.surveyManager.answers = answers; // getAnswer等で使用するため設定
