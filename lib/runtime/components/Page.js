...
class Page extends Component {
  ...
  getElementsValue(name) {
    const elements = Array.prototype.slice.call(this.pageEl.querySelectorAll(`[name="${name}"`));
  ...
  showValidationResult(name, validationResult) {
    ...
    const previousBalloon = document.body.querySelector(`.validation-error-balloon[data-target="${name}"]`);
    if (previousBalloon !== null) {
      document.body.removeChild(previousBalloon);
    }
    // errも削除
    ...
    balloon.appendChild(document.createTextNode(validationResult[0]));
