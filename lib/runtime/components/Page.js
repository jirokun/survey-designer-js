...
class Page extends Component {
  ...
  parsley() {
    ...
    Parsley.on('field:error', (fieldInstance) => {
      ...
      const $el = this.findTooltipEl(fieldInstance);
  ...
  handleClickNext() {
    ...
    // namesに対応する値を取得
    const answers = this.getInputValues();
    submitPage(answers);
  }
  ...
  render() {
    ...
    const nodeStack = state.getNodeStack();
    const answers = state.getInputValues();
    const backButtonStyle = {
      visibility: nodeStack.size === 0 ? 'hidden' : 'visible',
    };
    return (
