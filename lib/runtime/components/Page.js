...
import ZeroPadding from './plain/ZeroPadding';
...
class Page extends Component {
  ...
  componentDidMount() {
    const { survey, options, runtime, doNotTransition } = this.props;
    ...
    if (!doNotTransition) this.parsley(SurveyJS);
    // ユーザの行動をトレースする
  ...
  initializePageComponent(SurveyJS) {
    ...
    this.zeroPadding = new ZeroPadding(pageEl, survey, page, SurveyJS).initialize();
  ...
  createQuestions() {
    ...
    if (page.isFreeMode()) {
      return <div className="questions" dangerouslySetInnerHTML={{ __html: page.getHtml() }} />;
    }
