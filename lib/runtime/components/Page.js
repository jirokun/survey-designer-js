...
class Page extends Component {
  ...
  handleClickNext() {
    const { answers, survey, page, submitPage, noTransition } = this.props;
    ...
    this.pageManager.fireValidate(survey, page, mergedAnswers)
      .then((results) => {
        ...
        if (
          ...
          const logicVariableResults = page.evaluateLogicVariable(allOutputDefinitionMap, mergedAnswers);
          submitPage(Object.assign(pageAnswers, logicVariableResults));
          return;
        }
  ...
  // JavaScriptを評価する
  evaluateJavaScript() {
    const { survey, page, runtime } = this.props;
  ...
  createQuestions() {
    ...
    return page.questions.map((q, index) => {
      ...
      const props = { id: key, question: q, page, replaceUtil: survey.createReplaceUtil(runtime.getAnswers().toJS()) };
