...
function clonePage(state, flowId, x, y) {
  const flow = Utils.findNode(state, flowId);
  const clonedNode = Utils.cloneObj(flow);
  clonedNode.id = Utils.generateNextId(state, 'flow');
  state.survey.nodes.push(clonedNode);
  state.survey.positionDefs.push({ flowId: clonedNode.id, x, y });
  return state;
}
function removeEdge(state, sourceNodeId, targetNodeId) {
  ...
  if (sourceNode.type === 'page') {
    ...
    throw `unkown flow type: ${sourceNode.type}`;
  }
  return state;
}
...
function connectNode(state, sourceNodeId, dstNodeId) {
  ...
  if (sourceNode.type === 'page') {
    ...
    // デフォルトはpage
    throw `unknown flow type: ${sourceNode.type}`;
  }
  return state;
}
...
function editorReducer(state, action) {
  switch (action.type) {
    case C.SELECT_FLOW:
      return state.setCurrentNodeId(action.flowId);
    case C.ADD_FLOW:
      return state.addNode(action.index, action.flowType);
    case C.CLONE_PAGE:
      return clonePage(state, action.flowId, action.x, action.y);
    case C.REMOVE_EDGE:
      return removeEdge(state, action.sourceNodeId, action.targetNodeId);
    case C.REMOVE_FLOW:
      return state.deleteNode(action.flowId);
