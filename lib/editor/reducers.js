...
function finisherSelector(survey, action) {
  const finisherIndex = survey.findPageIndex(action.finisherId);
...
function pageReducer(survey, action, replaceUtil) {
  switch (action.type) {
    case C.ADD_ITEM:
    ...
    case C.ADD_LOGIC_VARIABLE:
      return survey.updateIn(pageSelector(survey, action), page => page.addLogicVariable(survey));
    case C.ADD_QUESTION:
    ...
    case C.CHANGE_LOGIC_VARIABLE:
      return survey.updateIn(pageSelector(survey, action), page => page.updateLogicVariable(action.logicVariableId, action.logicVariable));
    ...
    case C.CHANGE_ITEM_ATTRIBUTE:
      return survey.updateIn(pageSelector(survey, action), page =>
        page.updateItemAttribute(action.questionId, action.itemId, action.attributeName, action.value, replaceUtil));
    case C.CHANGE_SUB_ITEM_ATTRIBUTE:
      return survey.updateIn(pageSelector(survey, action), page =>
        page.updateSubItemAttribute(action.questionId, action.itemId, action.attributeName, action.value, replaceUtil));
    case C.CHANGE_QUESTION_ATTRIBUTE:
      return survey.updateIn(pageSelector(survey, action), page =>
        page.updateQuestionAttribute(action.questionId, action.attributeName, action.value, action.subValue, replaceUtil));
    case C.REMOVE_LOGIC_VARIABLE:
      return survey.updateIn(pageSelector(survey, action), page => page.removeLogicVariable(action.logicVariableId));
    case C.REMOVE_ITEM:
...
function finisherReducer(survey, action, replaceUtil) {
  switch (action.type) {
    case C.CHANGE_FINISHER_ATTRIBUTE:
      return survey.updateIn(finisherSelector(survey, action), finisher =>
        finisher.updateFinisherAttribute(action.attributeName, action.value, replaceUtil));
    default:
      return survey;
  }
}
...
function surveyReducer(survey, action, replaceUtil) {
  ...
  newSurvey = pageReducer(newSurvey, action, replaceUtil);
  newSurvey = finisherReducer(newSurvey, action, replaceUtil);

  // survey自体の更新
  switch (action.type) {
    case C.CHANGE_FINISHER_ATTRIBUTE:
      return finisherReducer(newSurvey, action.finisherId, action, replaceUtil);
    case C.REMOVE_NODE:
...
export default function reducer(state, action) {
  ...
  const replaceUtil = state.getSurvey().createReplaceUtil(
