...
function editorReducer(state, action) {
  switch (action.type) {
    ...
    case C.ADD_LOGIC_VARIABLE:
      return state.addLogicVariable(action.pageId);
    case C.ADD_NODE:
      return state.addNode(action.index, action.nodeType);
    case C.REMOVE_CHILD_CONDITION:
      return state.removeChildCondition(action.branchId, action.conditionId, action.childConditionId);
    case C.REMOVE_CONDITION:
      return state.removeCondition(action.branchId, action.conditionId);
    case C.REMOVE_ITEM:
      return state.removeItem(action.pageId, action.questionId, action.itemId);
    case C.REMOVE_LOGIC_VARIABLE:
      return state.removeLogicVariable(action.pageId, action.logicVariableId);
    case C.REMOVE_NODE:
      return state.removeNode(action.nodeId);
    case C.REMOVE_QUESTION:
      return state.removeQuestion(action.pageId, action.questionId);
    case C.SWAP_CONDITION:
      return state.swapCondition(action.nodeId, action.srcConditionId, action.destConditionId);
    case C.SWAP_ITEM:
    ...
    case C.CHANGE_LOGIC_VARIABLE:
      return state.updateLogicVariable(action.pageId, action.logicVariableId, action.logicVariable);
    case C.CHANGE_PAGE_ATTRIBUTE:
      return state.updatePageAttribute(action.pageId, action.attributeName, action.value);
    case C.CHANGE_TITLE:
    ...
    case C.CHANGE_PANEL:
      return state.updatePanel(action.panel);
    ...
    case C.CHANGE_SAVE_SURVEY_STATUS:
      return state.updateSaveSurveyStatus(action.saveSurveyStatus);
    case C.ADD_QUESTION:
