...
function questionReducer(survey, action) {
  switch (action.type) {
...
function surveyReducer(survey, action, replacer) {
  ...
  const newSurvey = branchReducer(survey, action) ||
    ...
    questionReducer(survey, action, replacer) ||
    ...
    (() => {
      switch (action.type) {
        ...
        case C.CHANGE_SURVEY_CSS_URLS:
          return survey.updateCssUrls(action.cssOption.getRuntimeUrls(), action.cssOption.getPreviewUrls());
        default:
          return survey;
      }
    })();
