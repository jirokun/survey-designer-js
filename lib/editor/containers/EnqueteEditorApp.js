...
import { isDevelopment, appendUrlParams } from '../../utils';
        ...
        <Frame
          ...
          initialContent={`
            ...
            <head>
              ...
              ${survey.getCssRuntimeUrls().toArray().map(url => `<link type="text/css" rel="stylesheet" href=${appendUrlParams(url, options.getCacheSuffix())} />`).join('\n')}
              ${survey.getCssPreviewUrls().toArray().map(url => `<link type="text/css" rel="stylesheet" href=${appendUrlParams(url, options.getCacheSuffix())} />`).join('\n')}
              ${survey.getCssDetailUrls().toArray().map(url => `<link type="text/css" rel="stylesheet" href=${appendUrlParams(url, options.getCacheSuffix())} />`).join('\n')}
            </head>
