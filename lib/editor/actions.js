...
export function saveSurvey(dispatch, saveSurveyUrl, survey) {
  $.ajax({
    ...
    dispatch(changeSaveSurveyStatus(C.SURVEY_POSTED_SUCCESS));
  }).fail(() => {
    dispatch(changeSaveSurveyStatus(C.SURVEY_POSTED_FAILED));
  });
  dispatch(changeSaveSurveyStatus(C.SURVEY_POSTING));
}
