  ...
  render() {
    ...
    return connectDragPreview(connectDropTarget(
      <li className={classNames('question', { invisible: dragging })}>
        {connectDragSource(<i className={classNames('fa fa-bars drag-handler', { invisible: page.isFreeMode() && !isDevelopment() })} />)}
        &nbsp;
        <a href="javascript:void(0)" onClick={() => this.handleQuestionSelect(question.getId())}>{strip(title)}</a>
        <div className={classNames('item-controller', { invisible: page.isFreeMode() && !isDevelopment() })}>
          <i
