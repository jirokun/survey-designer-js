...
class PageInFlow extends Component {
  ...
  createQuestionPopover() {
    ...
    const questions = [
      ...
      { dataType: 'Radio', label: '単一選択肢(ラジオボタン)' },
      { dataType: 'Select', label: '単一選択肢(プルダウン)' },
      { dataType: 'MultiNumber', label: '数値記入' },
      { dataType: 'SingleText', label: '1行テキスト' },
      { dataType: 'Text', label: '複数行テキスト' },
      { dataType: 'Matrix', label: '表形式' },
      { dataType: 'Prefecture', label: '都道府県' },
      { dataType: 'Description', label: '説明文 / 表紙など' },
    ...
    return (
      <Popover id="popover-positioned-right" title="ページへのアイテム追加">
        {questions.map(question => (
          <Button
            key={`PageInFlow_${question.dataType}`}
            bsSize="small"
            bsStyle="default"
  ...
  /** 設問を追加するためのエレメントを作成 */
  createInsertElement(page, key, index) {
    return (
      <OverlayTrigger
        key={key} 
        rootClose
      ...
      >
        <li className={classNames('insert-item-box', { invisible: page.isFreeMode() && !isDevelopment() })}>
          <div>
  ...
  render() {
    ...
    if (questions.size === 0) {
      const sizeZeroKey = `PageInFlow_${page.getId()}__0}`;
      questionEls.push(this.createInsertElement(page, `${sizeZeroKey}_insert_0`, 0));
    }
    questions.forEach((question, i) => {
      const key = `PageInFlow_${page.getId()}_${question.getId()}`;
      if (i === 0) {
        questionEls.push(this.createInsertElement(page, `${key}_insert_${i}`, i));
      }
      questionEls.push(<QuestionInFlow key={key} node={node} page={page} question={question} />);
      questionEls.push(this.createInsertElement(page, `${key}_insert_${i + 1}`, i + 1));
    });
    const pageTitle = `${survey.calcPageLabel(page.getId())}${page.isFreeMode() ? ' (編集不可)' : ''}`;
    return (
