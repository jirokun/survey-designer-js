...
class NodeInFlowOrig extends Component {
  ...
  createTransitionSource(node) {
    const { survey, selectNode } = this.props;
    const sourceNodes = node.findSourceNodes(survey);
    const sourceNodeList = sourceNodes.map((sourceNode) => {
    const currentNodeIndex = survey.findNodeIndex(node.getId());
      const key = `source_${sourceNode.getId()}_${node.getId()}`;
      if (sourceNode.isPage()) {
        const pageNo = survey.calcPageNo(sourceNode.getRefId());
        return (
          <li key={key}>
            <span className="btn btn-link" onClick={() => selectNode(sourceNode.getId())}>ページ {pageNo}</span>
          </li>
        );
      }
      const branchNo = survey.calcBranchNo(sourceNode.getRefId());
      const sourceNodeIndex = survey.findNodeIndex(sourceNode.getId());

      if (currentNodeIndex < sourceNodeIndex) return null;
      const beforeNode = survey.getNodes().get(sourceNodeIndex - 1);
      if (beforeNode && beforeNode.isPage()) {
        const beforePageNo = survey.calcPageNo(beforeNode.getRefId());
        return (
          <li key={key}>
            <span className="btn btn-link" onClick={() => selectNode(sourceNode.getId())}>分岐 {branchNo}</span>
            <span className="btn btn-link" onClick={() => selectNode(beforeNode.getId())}>(ページ {beforePageNo})</span>
          </li>
        );
      }
      return (
    ...
    return (
      <Popover id={`transition-source-${node.getId()}`} title="遷移元">
        <ul className="source-node-list">{sourceNodeList}</ul>
      </Popover>
    );
  }
  ...
  /** 描画 */
  render() {
    const { survey, runtime, node, nodeLabel, dragging } = this.props;
    ...
    const focusThisNode = currentNode && currentNode.getId() === node.getId();
    const isFreeMode = node.isPage() && survey.findPageFromNode(node.getId()).isFreeMode();
    return connectDragPreview(connectDropTarget(
      <div className={classNames(node.getType(), { invisible: dragging, selected: focusThisNode, 'free-edit': isFreeMode }, 'enq-page')} onClick={() => this.onNodeSelect(node)}>
        {connectDragSource(<i className="fa fa-bars drag-handler" />)}
        &nbsp;
        <span className="node-title">{nodeLabel}</span>
        {this.props.children}
        <div className={classNames('item-controller enq-item-controller', { invisible: !deletable })}>
          <OverlayTrigger
            ...
            trigger="click"
            placement="right"
            overlay={this.createTransitionSource(node)}
          >
            <span className="btn btn-link btn-xs">遷移元</span>
          </OverlayTrigger>
