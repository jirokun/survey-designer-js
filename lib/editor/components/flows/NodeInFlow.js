  ...
  createTransitionSource(node) {
    ...
    const sourceNodeList = sourceNodes.map((sourceNode) => {
      ...
      if (sourceNode.isPage()) {
        ...
        return <li key={key} className="btn btn-link">ページ {pageNo}</li>;
      }
      ...
      if (beforeNode && beforeNode.isPage()) {
        ...
        return <li key={key} className="btn btn-link">分岐 {branchNo} (ページ {beforePageNo})</li>;
      }
      return <li key={key} className="btn btn-link">分岐 {branchNo}</li>;
    });
    return (
