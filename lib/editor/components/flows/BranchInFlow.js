...
class BranchInFlow extends Component {
  ...
  render() {
    ...
    if (conditions.size === 0) {
      const sizeZeroKey = `BranchInFlow_${branch.getId()}__0}`;
      conditionEls.push(this.createInsertElement(`${sizeZeroKey}_insert_0`, 0));
    }
    conditions.forEach((condition, i) => {
      const key = `BranchInFlow-${condition.getId()}`;
    ...
    if (branch.getNextNodeId()) {
      ...
      if (followingPageNodeIds.find(nodeId => branch.getNextNodeId() === nodeId)) {
        elseLabel = survey.calcNodeLabel(branch.getNextNodeId());
      } else {
        elseLabel = '不正なページ';
      }
    } else {
      if (node.getNextNodeId()) {
        elseLabel = survey.calcNodeLabel(node.getNextNodeId());
      } else {
        elseLabel = '不正なページ';
      }
    }
      ...
      <NodeInFlow node={node} nodeLabel={nodeLabel} className="condition">
        <ul className="conditions">
          ...
          <li className="condition">
            ...
            &nbsp;
            {conditions.size > 0 ? '上記条件以外は' : ''}{elseLabel}に遷移</li>
        </ul>
      </NodeInFlow>
    );
  }
