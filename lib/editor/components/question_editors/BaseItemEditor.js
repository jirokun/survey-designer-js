...
export default class BaseItemEditor extends Component {
  ...
  handleClickAddButton() {
    ...
    if (question.isMatrixHtmlEnabled()) return;
    if (subItem) {
  ...
  createAdditionalInput() {
    const { item, question, disabled } = this.props;
    ...
    if (additionalInput) {
      elements.push(
        <td key={`${baseKey}_1`} className="option">
          <select disabled={disabled || question.isMatrixHtmlEnabled()} value={item.getAdditionalInputType()} onChange={e => this.handleChangeOption('additionalInputType', e.target.value)}>
      ...
      if (!question.getMatrixType()) {
        elements.push(<td key={`${baseKey}_2`} className="option"><input type="text" disabled={disabled} className="item-editor-unit" value={item.getUnit()} onChange={e => this.handleChangeOption('unit', e.target.value)} /></td>);
      }
      elements.push(
        <td key={`${baseKey}_3`} className="option">
          <input type="checkbox" disabled={disabled || question.isMatrixHtmlEnabled()} className="additional-input" onChange={e => this.handleChangeOption('additionalInput', e.target.checked)} checked={item.hasAdditionalInput()} />
        </td>,
      );
      return elements;
    }
    ...
    } else if (question.getMatrixType() && !additionalInput) {
      elements.push(<td key={`${baseKey}_2`} />);
    }
    elements.push(
      <td key={`${baseKey}_3`} className="option" style={{ whiteSpace: 'nowrap' }}>
        <input disabled={disabled || question.isMatrixHtmlEnabled()} type="checkbox" className="additional-input" onChange={e => this.handleChangeOption('additionalInput', e.target.checked)} checked={item.hasAdditionalInput()} />
      </td>,
    );
    return elements;
  }
  ...
  /** 数値項目の単位をレンダリングする */
  createUnitLabel() {
    const { page, question, item, createsUnitLabel } = this.props;
    if (!createsUnitLabel) return null;
    
    const elements = [];
    const baseKey = `BaseItemEditor_${item.getId()}`;
    
    const disabled = page.isFreeMode() || question.isMatrixHtmlEnabled();
    elements.push((
      <td
        key={`${baseKey}_1`}
        className="option"
      >
        <input
          className={classNames('item-editor-unit', { disabled })}
          disabled={disabled}
          value={item.getUnit()}
          onChange={e => this.handleChangeOption('unit', e.target.value)}
        />
      </td>
    ));
    return elements;
  ...
  createOptionalItem() {
    const { page, question, item, optionalItem } = this.props;
    if (!optionalItem) return null;

    const baseKey = `BaseItemEditor_${item.getId()}`;
    const disabled = page.isFreeMode() || question.isMatrixHtmlEnabled();
    return (
      <td
        key={`${baseKey}_optionalItem`}
        className="option"
      >
        <input
          disabled={disabled}
          ...
          onChange={e => this.handleChangeOption('optional', e.target.checked)}
  ...
  render() {
    ...
    const editor = plainText || disabled ? (<input
      disabled={disabled}
      type="text"
      ...
      (
        <td className="option">
          <input
            disabled={disabled}
            type="checkbox"
            className="random-fixed"
            onChange={e => this.handleChangeOption('randomFixed', e.target.checked)}
            checked={item.isRandomFixed()}
          />
        </td>
      ) : null;
      ...
      (
        <td className="option">
          <input
            ...
            type="checkbox"
            className="exclusive"
            onChange={e => this.handleChangeOption('exclusive', e.target.checked)}
    ...
    return connectDragPreview(connectDropTarget(
      <tr className={classNames('item-editor-row', { invisible: dragging })}>
        <td className="item-editor-dnd">{connectDragSource(<i className={classNames('fa fa-bars drag-handler', { invisible: disabled || question.isMatrixHtmlEnabled() })} />)}</td>
        ...
        <td className="buttons">
          <Glyphicon
            className={classNames('clickable icon-button', { invisible: disabled || question.isMatrixHtmlEnabled() })}
            glyph="plus-sign"
            onClick={() => this.handleClickAddButton()}
          />
          <Glyphicon
            className={classNames('clickable icon-button', { invisible: items.size === 1 || disabled || question.isMatrixHtmlEnabled() })}
            glyph="remove-sign"
