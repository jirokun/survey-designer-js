...
class NumberValidationEditorPart extends Component {
  ...
  componentDidMount() {
    const hotRootEl = this.hotTable.hotInstance.rootElement;
  ...
  /** handsontableの行番号と列蛮行から対象となるoutputDefinitionを取得する */
  ...
  handleAfterChange(changes, source) {
    if (source === 'loadData' || !this.hotTable) return;
  ...
  resizeHotTable() {
    ...
    const height = $(this.hotRootEl).find('.ht_master .wtHider').height();
    this.hotTable.hotInstance.updateSettings({ height });
  }
  ...
  createHotTableForMatrix() {
    ...
    return (
      <HotTable
        ref={(hotTable) => { this.hotTable = hotTable || this.hotTable; }}
