...
class NumberValidationEditorPart extends Component {
  ...
  findOutputDefinitionFrom(row, col) {
    ...
    if (this.isMatrixNumber()) {
      if (col >= question.getSubItems().size) { // 行の合計値の場合
        ...
        const outputDefinitionForSums = question.getOutputDefinitionForSum('rows', pageNo, survey.calcQuestionNo(page.getId(), question.getId()));
        return outputDefinitionForSums.get(row);
      } else if (row >= question.getItems().size) { // 列の合計値の場合
        const pageNo = survey.calcPageNo(page.getId());
        const outputDefinitionForSums = question.getOutputDefinitionForSum('columns', pageNo, survey.calcQuestionNo(page.getId(), question.getId()));
  ...
  createSettingForMatrixNumber() {
    ...
    const rowSumOutputDefinitions = question.getOutputDefinitionForSum('rows', pageNo, questionNo);

    // 列の合計のOutputDefinitionを用意
    const colSumOutputDefinitions = question.getOutputDefinitionForSum('columns', pageNo, questionNo);
