...
class ItemEditorPart extends Component {
  ...
  componentWillUnmount() {
    ...
    this.destroyed = true;
    return Array.prototype.map.call(editorEls, el => ItemEditorPart.getTinyMCEEditorFromEl(el).destroy());
  }
  ...
  render() {
    ...
    const replaceUtil = survey.createReplaceUtil(runtime.getAnswers().toJS());
    ...
    const editor = plainText ? (<input
      ...
      value={replaceUtil.name2OutputNo(content)}
    />)
        : (<TinyMCE
          ...
          content={replaceUtil.name2OutputNo(content)}
        />);
