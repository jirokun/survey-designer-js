...
class IndividualNumberValidationRuleEditorPart extends Component {
  ...
  componentWillUnmount() {
    ...
    // セルを更新するため次のtickでonUpdateを実行する
    setTimeout(() => {
      this.props.onUpdate(outputDefinitionId, numberValidationRule.getId());
    }, 1);
  }
  ...
  render() {
    const { numberValidationRule } = this.state;
    const validationCount = numberValidationRule.getNumberValidations().size;
