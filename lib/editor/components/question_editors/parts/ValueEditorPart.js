...
class ValueEditorPart extends Component {
  ...
  render() {
    ...
    const options = survey.findPrecedingOutputDefinition(runtime.findCurrentNode(survey).getId()).map((od) => {
      const key = `${this.constructor.name}_${attr}_${od.getId()}`;
      return <option key={key} value={`\${${od.getId()}#answer}`}>{od.getOutputNo()} {od.getLabel()}</option>;
    }).toList();
    /*
    const formEl = this.state.mode === 'fixedValue' ?
      <FormControl type="number" value={value} onChange={e => this.handleChangeQuestionAttribute(e.target.value)} />
