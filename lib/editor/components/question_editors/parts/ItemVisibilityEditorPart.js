...
class ItemVisibilityEditorPart extends Component {
  ...
  outputDefinitionIdSelectOptions() {
    ...
    const node = survey.findNodeFromRefId(page.getId());
    const options = {};
    const outputDefinitions = survey.findPrecedingOutputDefinition(node.getId(), false).filter(od => (
      od.getOutputType() === 'number' ||
      od.getOutputType() === 'checkbox' ||
      od.getOutputType() === 'radio' ||
      od.getOutputType() === 'select'
    ));
    outputDefinitions.forEach((od) => {
  ...
  outputDefinitionIdRenderer(instance, td, row, col, prop, value) {
    ...
    const node = survey.findNodeFromRefId(page.getId());
    const targetOutputDefinition = survey.findPrecedingOutputDefinition(node.getId(), false).find(od => od.getId() === value);
    // 見つからなかった場合削除されている
