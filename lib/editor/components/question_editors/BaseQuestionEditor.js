...
import { Popover, OverlayTrigger, Button, Col, FormGroup, ControlLabel, Checkbox, FormControl } from 'react-bootstrap';
...
class BaseQuestionEditor extends Component {
        ...
        { random || showTotal ?
          <FormGroup>
            ...
            <Col md={10}>
              ...
              { matrixColVerticalWriting ?
                <Checkbox checked={question.isMatrixColVerticalWriting()} onChange={e => this.handleChangeQuestionAttribute('matrixColVerticalWriting', e.target.checked)}>{matrixColVerticalWriting === true ? '列ヘッダを縦書き' : matrixColVerticalWriting}</Checkbox> : null }
            </Col>
          </FormGroup> : null }
        { item ?
          <FormGroup>
            <div className="col-md-2 control-label">
              ...
              <div>
                ...
                >
                  <Button bsSize="xsmall" bsStyle="link">表示制御</Button>
                </OverlayTrigger>
              </div>
              <div>
                <OverlayTrigger
                ...
                <table className="item-editor-table">
                  <thead>
                    <tr>
                      ...
                      {subItemAdditionalInput && subItems.find(i => i.hasAdditionalInput()) ? <th>入力<br />タイプ</th> : <th />}
                      {subItemAdditionalInput && subItems.find(i => i.hasAdditionalInput()) ? <th>単位</th> : <th />}
  ...
  createValidationEditors(numberTypeOutputDefinitions) {
    const {
      page,
      question,
      checkCount,
    ...
    if (!this.state.showValidationEditors) return null;
      ...
      <div>
        ...
        { numberTypeOutputDefinitions.size !== 0 ?
          <FormGroup>
            ...
            <Col md={10}>
              <NumberValidationEditorPart items={question.getItems()} {...this.props} />
            </Col>
          </FormGroup> : null }
      </div>
    );
  ...
  createValidationContainer() {
    ...
    return (
      <form>
        ...
        {this.createValidationEditors(numberTypeOutputDefinitions)}
      </form>
    );
  }
