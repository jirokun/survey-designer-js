            ...
            <Col md={10}>
              ...
              { matrixColumnUniqueness ?
                <Checkbox
                  ...
                  disabled={disabled}
                  className={classNames({ disabled })}
                  checked={question.isMatrixColumnUniqueness()}
                  onChange={e => this.handleChangeQuestionAttribute('matrixColumnUniqueness', e.target.checked)}
                >{matrixColumnUniqueness === true ? '縦方向にユニーク制約をつける' : matrixColumnUniqueness}</Checkbox> : null }
            </Col>
          </FormGroup> : null }
  ...
  createCheckCountValidationEditor() {
    ...
    if (question.dataType === 'Matrix') {
      if (question.isMatrixReverse()) {
        return question.getSubItems().map((subItem, i) => {
        ...
        return question.getItems().map((item, i) => {
          return this.createCheckCountValidationEditorRow(
