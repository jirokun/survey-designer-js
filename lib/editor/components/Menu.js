...
class Menu extends Component {
  ...
  createDevelopemtnMenu() {
    ...
    return [
      <NavItem key="menu-development" className="menu-development" onClick={() => this.setState({ showDevelopmentMenu: true })}>開発モードメニュー</NavItem>,
      (<Modal key="dialog-development" show={this.state.showDevelopmentMenu} onHide={() => this.setState({ showDevelopmentMenu: false })}>
        <Modal.Body>
          <div><a className="btn btn-primary" href={dataStr} download={fileName}>定義の保存</a></div>
          <div>定義の読み込み<input type="file" onChange={e => this.handleFileChange(e)} /></div>
        </Modal.Body>
      </Modal>),
    ];
/*
        <MenuItem eventKey={1.1} href={dataStr} download={fileName}></MenuItem>
        <MenuItem eventKey={1.2}><input type="file" />定義の保存</MenuItem>
      </NavDropdown>
      */
  }
