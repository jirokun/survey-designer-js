...
import SurveySettingEditor from './editors/SurveySettingEditor';
...
class Menu extends Component {
  ...
  handleValidation() {
    const { survey } = this.props;
    const errors = survey.validate().filter(error => error.getType() === ERROR_TYPES.ERROR);
    if (errors.size !== 0) {
      alert(errors.map(error => error.getMessage()).join('\n'));
    } else {
  ...
  createDevelopmentMenu() {
    ...
    return [
      <NavDropdown key="menu-development" className="menu-development" eventKey={2} title="開発モードメニュー" id="basic-nav-dropdown">
        <NavItem eventKey={2.1} key="menu-development-export" onClick={() => this.setState({ showDevelopmentMenu: true })}>アンケート定義の保存・読込</NavItem>
    ...
    return (
      <Navbar fluid className="enq-navbar">
        ...
        <Nav pullRight className="enq-navbar-nav">
          ...
          <NavDropdown eventKey={1} title="表示" id="view-dropdown">
            <MenuItem
              eventKey={1.1}
              href="#"
          ...
          <NavDropdown className="menu-preview" eventKey={1} title="プレビュー" id="preview-dropdown">
          ...
          <NavDropdown className="menu-application" eventKey={3} title="申請" id="application-dropdown">
