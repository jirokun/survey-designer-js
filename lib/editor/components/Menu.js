...
class Menu extends Component {
  ...
  handleClickShowDetail() {
    const { options } = this.props;
    const url = options.getShowDetailUrl(isDevelopment());
  ...
  handleChangeCssUrls(event) {
    const { options, changeSurveyCssUrls } = this.props;

    const title = event.target.value;
    const cssUrls = options.getCssUrlsByTitle(title);
    changeSurveyCssUrls(cssUrls);
  }
  ...
  createConfigMenu() {
    ...
    const selected = options.getCurrentCssTitle(survey.getCssUrls()) || options.getCssOptions().first().get('title');
    const selectOptions = options.getCssOptions().map(opt => opt.get('title')).toArray();
    return [
      <NavItem key="menu-config" className="menu-config" onClick={() => this.setState({ showConfig: true })}>設定</NavItem>,
      (<Modal key="dialog-config" show={this.state.showConfig} onHide={() => this.setState({ showConfig: false })}>
        <Modal.Body>
          <div>CSSの選択：
            <select value={selected} onChange={e => this.handleChangeCssUrls(e)}>
    ...
    return (
      <Navbar fluid className="enq-navbar">
        ...
        <Nav pullRight className="enq-navbar-nav">
          ...
          <NavDropdown className="menu-preview" eventKey={3} title="配信依頼確認" id="basic-nav-dropdown">
            <MenuItem eventKey={3.1} className="menu-delivery-confirm" onClick={() => this.handleApplication()}>配信依頼確認</MenuItem>
            <MenuItem eventKey={3.1} className="menu-validation" onClick={() => this.handleValidation()}>検証</MenuItem>
          </NavDropdown>

          {this.createConfigMenu()}
          {this.createDevelopemtnMenu()}
        </Nav>
      </Navbar>
    );
  }
...
const actionsToProps = dispatch => ({
  ...
  changeSurveyCssUrls: cssUrls => dispatch(Action.changeSurveyCssUrls(cssUrls)),
});
