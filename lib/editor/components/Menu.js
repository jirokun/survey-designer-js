    ...
    return (
      <Navbar fluid className="enq-navbar">
        ...
        <Nav pullRight className="enq-navbar-nav">
          {surveyPostStatus ? <NavItem>{surveyPostStatusLabel}</NavItem> : null}
          ...
          <NavItem onClick={() => this.handleValidation()}>バリデーション</NavItem>
          <NavDropdown eventKey={1} title="プレビュー" id="basic-nav-dropdown">
            <MenuItem eventKey={1.1} href="#" onClick={() => this.handleClickPreview()}>動作プレビュー</MenuItem>
            <MenuItem eventKey={1.1} href="#" onClick={() => this.handleClickShowDetail()}>詳細プレビュー</MenuItem>
          </NavDropdown>
