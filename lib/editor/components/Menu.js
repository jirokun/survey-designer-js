...
class Menu extends Component {
  ...
  handleChangeCssUrls(event) {
    ...
    const cssOption = options.getCssOptionByTitle(title);
  ...
  createConfigMenu() {
    ...
    if (!options.hasCssOptions()) { return <div />; }
    let title = options.getCssTitleByUrls(survey.getCssRuntimeUrls(), survey.getCssPreviewUrls());
    if (title === null) { title = options.getCssOptions().first().getTitle(); }
    const selectOptions = options.getCssOptions().map(opt => opt.getTitle()).toArray();
    return [
      ...
      (<Modal key="dialog-config" show={this.state.showConfig} onHide={() => this.setState({ showConfig: false })}>
        <Modal.Body>
          <div>CSSの選択：
            <select value={title} onChange={e => this.handleChangeCssUrls(e)}>
...
const actionsToProps = dispatch => ({
  ...
  changeSurveyCssOption: cssUrls => dispatch(Action.changeSurveyCssOption(cssUrls)),
});
