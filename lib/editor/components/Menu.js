...
class Menu extends Component {
  ...
  render() {
    ...
    const showGraphPane = viewSetting.getGraphPane();
    const showEditorPane = viewSetting.getEditorPane();
    const showPreviewPane = viewSetting.getPreviewPane();
        ...
        <Nav pullRight>
          ...
          <NavDropdown eventKey={1} title="表示" id="basic-nav-dropdown">
            <MenuItem
              eventKey={1.1}
              href="#"
              onClick={() => changeShowPane('graphPane', !showGraphPane)}
            >
              <Glyphicon glyph="ok" className={classNames({ invisible: !showGraphPane })} /> ページ一覧
            </MenuItem>
            <MenuItem
              eventKey={1.2}
              href="#"
