...
class Menu extends Component {
  handleSave() {
    const { state, saveSurvey } = this.props;
    const survey = state.getSurvey();
    console.log(JSON.stringify(survey.toJS()));
    return $.ajax({
      method: 'post',
      url: ENV.SAVE_URL.replace('${id}', survey.getId()),
      data: { _doc: survey.toJS() },
    }).then((result) => {
      console.log(result);
      //saveSurvey(survey.getId(), survey);
    });
  }
