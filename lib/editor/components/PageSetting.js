...
class PageSetting extends Component {
  ...
  render() {
    ...
    const page = state.findPageFromFlow(state.getCurrentFlowId());
    // ページが見つからない場合は描画しない(branchの場合)
    if (!page) {
      const branch = state.findBranchFromFlow(state.getCurrentFlowId());
      if (branch) {
        return <span>Disabled PageSetting Tab when branch is selected</span>;
      } else {
        throw `invalid currentFlowId: ${state.getCurrentFlowId()}`;
      }
    }
    return (
