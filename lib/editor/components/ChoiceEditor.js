...
class ChoiceEditor extends Component {
  ...
  renderChoiceEditorRow(choice, index, choices) {
    ...
    const randomFixed = question.isRandom() ?
      <td className="option"><input type="checkbox" className="random-fixed" onChange={() => this.handleChangeOption()} /></td> : null;

    return (
      <tr className="choice-editor-row" key={`choice-editor-row-${index}`}>
        <td className="choice-editor-tinymce-container">{editor}</td>
        {randomFixed}
        <td className="option"><input type="checkbox" className="free-input" onChange={() => this.handleChangeOption()} /></td>
        <td className="option"><input type="checkbox" className="free-input-required" onChange={() => this.handleChangeOption()} /></td>
        <td className="option"><input type="checkbox" className="exclusion" onChange={() => this.handleChangeOption()} /></td>
    ...
    return (
      <div ref={(el) => { this.rootEl = el; }} className="choice-editor">
        <table className="choice-editor-table">
          ...
          <tbody>
            {choices.map((choice, index, choices) => this.renderChoiceEditorRow(choice, index, choices))}
          </tbody>
        </table>
      </div>
    );
