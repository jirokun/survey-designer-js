...
class Graph extends Component {
  ...
  addPagePopover() {
    const { addFlow } = this.props;
    const { popoverIndex } = this.state;
    return (
      <Popover id="popover-positioned-right" title="ページ・分岐の追加">
        <Button bsSize="small" bsStyle="info" block onClick={() => addFlow(popoverIndex, 'page')}>ページ</Button>
        <Button bsSize="small" bsStyle="info" block onClick={() => addFlow(popoverIndex, 'branch')}>分岐</Button>
      </Popover>
    );
  }
  ...
  render() {
    ...
    flowDefs.forEach((flow, i) => {
      ...
      pageEls.push(this.createInsertEl(`${key}_arrow_${i + 1}`, i + 1, i !== flowDefs.size - 1));
    });
...
const actionsToProps = dispatch => ({
  addFlow: (index, flowType) => dispatch(Action.addFlow(index, flowType)),
});
