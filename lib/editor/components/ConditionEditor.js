...
class ConditionEditorOrig extends Component {
  ...
  renderQuestionOptions() {
    ...
    const optionValues = List(precedingPageNodeIds).map((nodeId) => {
      ...
      const valueLabels = page.getQuestions().map((question) => {
        const questionNo = state.calcQuestionNo(page.getId(), question.getId());
        const plainTitle = question.getPlainTitle();
        const optionLabel = `${questionNo}. ${plainTitle.substr(0, 10)}${plainTitle.length > 10 ? '...' : ''}`;
        return <option value={question.getId()}>{optionLabel}</option>;
      }).toList();
      return valueLabels;
    }).flatten(true);
    return optionValues;
  ...
  renderChildCondition(childCondition, index, childConditions) {
    ...
    return (
      <div ref={node => this.root = node} key={`child-conditions-${index}`} className="condition-editor">
        <select className="form-control condition-ref-id">
          {this.renderQuestionOptions()}
        </select>
        <span>の値が</span>
  ...
  renderPageOptions() {
    ...
    return followingPageNodeIds.map((nodeId) => {
      const page = state.findPageFromNode(nodeId);
      return <option value={nodeId}>{state.calcPageLabel(page.getId())}</option>;
    });
  }

  renderIf() {
    const { state, condition, isDragging } = this.props;
    ...
    return (
      <Well className="branch-editor" style={{ opacity }}>
        <div className="branch-editor-header">
          ...
          <span>を満たす場合</span>
          <select className="form-control condition-next-node-id">
