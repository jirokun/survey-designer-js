...
class ConditionEditorOrig extends Component {
  ...
  validateCondition(nextCondition) {
    ...
    const validationState = {
      isNextNodeIdValid: !!Utils.findNode(state, nextCondition.nextNodeId),
      isChildConditionsValid: nextCondition.childConditions.map(cc => !!Utils.findQuestionByStr(state, cc.refQuestionId)),
    };
    if (!validationState.isNextNodeIdValid || !validationState.isChildConditionsValid.some(s => !s)) {
    ...
    return (
      <Well className="branch-editor" style={{ opacity }}>
        <div className="branch-editor-header">
          ...
          <span>を満たす場合</span>
          <input ref={node => this.conditionNextNodeIdEl = node} type="text" className="form-control condition-next-flow-id" value={condition.nextNodeId} readOnly />
          <span>に遷移する</span>
        </div>
    ...
    return (
      <Well className="branch-editor" style={{ opacity }}>
        <div className="branch-editor-header">
          <span>上記以外の場合</span>
          <input
            ref={node => this.conditionNextNodeIdEl = node} type="text"
            className="form-control condition-next-flow-id" value={condition.nextNodeId} readOnly
          />
          <span>に遷移する</span>
        </div>
      </Well>
