...
class ConditionEditorOrig extends Component {
  ...
  /** 条件の値を入力するフォームを表示する */
  renderConditionValue(attr, value) {
  }
  ...
  renderChildCondition(childCondition, index, childConditions) {
    return (
      <div ref={(el) => { this.root = el; }} key={`child-conditions-${index}`} className="condition-editor">
        <select className="form-control condition-ref-id" onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'outputId', e.target.value)}>
        ...
        <input
          type="text"
          className="form-control condition-ref-value"
          placeholder="値"
          value={childCondition.getValue()}
          onChange={e => this.handleChangeCondition('value', e.target.value)}
        />
        <select className="form-control condition-ref-operator" value={childCondition.getOperator()} onChange={() => this.handleChangeChildCondition('operator')}>
        ...
        <Glyphicon className="clickable icon-button text-info" glyph="plus-sign" onClick={() => this.handleClickAddButton(index)} />
        {
          childConditions.length === 1 ?
            ...
            <Glyphicon
              ...
              glyph="minus-sign"
              onClick={() => this.handleClickMinusButton(index)}
            />
        }
      </div>
    );
