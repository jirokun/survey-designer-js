...
class ConditionEditorOrig extends Component {
  ...
  handleClickMinusButton(childCondition) {
  ...
  renderConditionValue(childCondition) {
    ...
    switch (outputType) {
      case 'text':
        return [
          (<span>の入力値が</span>),
          (<input type="text" className="form-control" value={childCondition.getValue()} onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'value', e.target.value)} />),
          (<span>と等しい</span>),
        ];
      case 'number':
        return [
          (<span>の入力値が</span>),
          (<input type="number" className="form-control" value={childCondition.getValue()} size="3" onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'value', e.target.value)} />),
          (<select className="form-control condition-ref-operator" value={childCondition.getOperator()} onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'operator', e.target.value)}>
      ...
      case 'checkbox':
        return [
          (<span>で</span>),
          (<select className="form-control" value={childCondition.getValue()} onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'value', e.target.value)}>
            <option value="" />
            <option value="off">を選択していない</option>
            <option value="on">を選択している</option>
          </select>),
        ];
      case 'select':
      case 'radio':
        ...
        return [
          (<span>で</span>),
          (<select className="form-control" onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'value', e.target.value)} value={childCondition.getValue()}>
            <option value="" />
  ...
  renderChildCondition(childCondition, index, childConditions) {
    const minusButtonStyle = childConditions.size === 1 ? { visibility: 'hidden' } : {};
    return (
      <div ref={(el) => { this.root = el; }} key={`child-conditions-${index}`} className="condition-editor">
        ...
        <Glyphicon
          ...
          glyph="minus-sign"
          onClick={() => this.handleClickMinusButton(childCondition)}
