...
class PageInGraph extends Component {
  ...
  render() {
    ...
    const questionEls = page.getQuestions().map((question) => {
      const title = question.get('title');
      return <li className="question"><button className="btn btn-info btn-xs">編集</button> {title}</li>;
    }).interpose(
      <li className="insert-item-box">
        <div>
          <OverlayTrigger trigger="click" rootClose placement="right" overlay={addQuestionPopover}>
