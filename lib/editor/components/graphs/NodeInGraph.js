...
class NodeInGraphOrig extends Component {
  ...
  /** 描画 */
  render() {
    const { state, node, nodeLabel, dragging } = this.props;
    const { connectDragPreview, connectDragSource, connectDropTarget } = this.props;
    const deletable = (node.isFinisher() && state.getFinishers().size > 1)
                        || node.isPage()
                        || node.isBranch(); // 最後のfinisher以外は削除可能
    return connectDragPreview(connectDropTarget(
      <div className={classNames(node.getType(), { invisible: dragging })} onClick={() => this.onNodeSelect(node)}>
        {connectDragSource(<i className="fa fa-bars drag-handler" />)}
        &nbsp;
        {nodeLabel}
        {this.props.children}
        <div className={classNames('item-controller', { invisible: !deletable })}>
