...
class MenuConfigModal extends Component {
  ...
  render() {
    ...
    if (!options.hasCssOptions()) { return null; }
    let id = options.getCssOptionIdByUrls(survey.getCssRuntimeUrls(), survey.getCssPreviewUrls());
    if (!id) { id = options.getCssOptions().first().getId(); }
