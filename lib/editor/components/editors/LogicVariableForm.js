...
class LogicVariableForm extends Component {
  ...
  /** LogicVariableの変更 */
  handleClickSave() {
    const { survey, logicVariable, page, changeLogicVariable } = this.props;
  ...
  render() {
    const { runtime, survey, page, logicVariable } = this.props;
    ...
    const precedingOutputDefinitions = survey.findPrecedingOutputDefinition(runtime.findCurrentNode(survey).getId())
      ...
      .filter(od => od.getName() !== logicVariable.getId());  // 自分自身は除く
    const editableVariableName = LogicVariableForm.getEditableVariableName(pageNo, variableName);
    return (
