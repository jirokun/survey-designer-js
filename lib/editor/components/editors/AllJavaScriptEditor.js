...
class AllJavaScriptEditor extends Component {
  ...
  componentDidMount() {
    window.addEventListener('beforeunload', (e) => {
      ...
      if (!allJavaScriptCode.isChangedCode(survey)) {
  ...
  saveAllJavaScript() {
    const { survey, saveAllJSCodeAndSavedAt } = this.props;
    const allJavaScriptCode = new AllJavaScriptCode({ code: this.state.code });
    ...
    return saveAllJSCodeAndSavedAt(allJavaScriptCode);
  }
...
const actionsToProps = dispatch => ({
  ...
  saveAllJSCodeAndSavedAt: allJavaScriptCode => Action.saveAllJSCodeAndSavedAt(dispatch, allJavaScriptCode, Date.now()),
});
