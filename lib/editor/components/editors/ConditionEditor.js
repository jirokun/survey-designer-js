    ...
    switch (outputType) {
      ...
      case 'number': {
        ...
        return [
          ...
          (<ValueEditorPart key={`${keyBase}-3`} value={childCondition.getValue()} onChange={value => this.handleChangeChildCondition(childCondition.getId(), 'value', value)} style={valueEditorStyle} />),
          (<ExSelect
            key={`${keyBase}-4`}
            className="form-control condition-ref-operator"
            value={childCondition.getOperator()}
