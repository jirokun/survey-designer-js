...
class ConditionEditor extends Component {
  ...
  handleClickAddButton(ccIndex) {
    const { runtime, survey, addChildCondition, condition } = this.props;
  ...
  handleClickRemoveButton(childCondition) {
    const { runtime, survey, removeChildCondition, condition } = this.props;
    const branch = runtime.findCurrentBranch(survey);
  ...
  handleChangeCondition(attr, value) {
    const { runtime, survey, condition, changeConditionAttribute } = this.props;
    const branch = runtime.findCurrentBranch(survey);
  ...
  handleChangeChildCondition(childConditionId, attr, value) {
    const { runtime, survey, condition, changeChildConditionAttribute } = this.props;
    const branch = runtime.findCurrentBranch(survey);
  ...
  createQuestionOptions(childCondition) {
    ...
    return survey.findPrecedingOutputDefinition(runtime.findCurrentNode(survey).getId())
    ...
    switch (outputType) {
      ...
      case 'number':
        return [
          ...
          (<ValueEditorPart key={`${keyBase}-3`} value={childCondition.getValue()} onChange={value => this.handleChangeChildCondition(childCondition.getId(), 'value', value)} style={{ width: '200px' }} />),
          ...
          (<select key={`${keyBase}-8`} className="form-control" onChange={e => this.handleChangeChildCondition(childCondition.getId(), 'value', e.target.value)} value={childCondition.getValue()}>
            <option value="" />
            {
              overrideItems ?
                ...
                question.getItems().map(item => <option key={`${questionId}_option_${item.getIndex() + 1}`} value={`{{${questionId}.choice.${item.getId()}.value}}`}>{item.getPlainLabel()}</option>).toArray()
            }
          </select>),
  ...
  /** 描画 */
  render() {
    const { condition, dragging } = this.props;
