...
class LogicalVariableForm extends Component {
  ...
  /** LogicalVariableの変更 */
  handleClickSave() {
    const { survey, logicalVariable, page, changeLogicalVariable } = this.props;
    ...
    const editableVariableName = LogicalVariableForm.getEditableVariableName(pageNo, variableName);
    const invalidChars = [
      { char: ' ', desc: '半角スペース' },
      { char: '#', desc: '半角シャープ' },
      { char: '{', desc: '半角左ブレース' },
