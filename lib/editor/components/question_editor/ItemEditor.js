...
class ItemEditor extends Component {
  ...
  handleChangeItem(plainLabel, html) {
    ...
    if (html) {
      changeItemAttribute(page.getId(), question.getId(), item.getId(), 'plainLabel', plainLabel);
      changeItemAttribute(page.getId(), question.getId(), item.getId(), 'label', html);
    } else {
      changeItemAttribute(page.getId(), question.getId(), item.getId(), 'plainLabel', plainLabel);
      changeItemAttribute(page.getId(), question.getId(), item.getId(), 'label', plainLabel);
    }
  }
  ...
  renderAdditionlInput() {
    const { question, item } = this.props;
    if (question.getDataType() === 'MultiNumber') return null;
    const additionalInput = item.hasAdditionalInput();
    const elements = [];
    if (additionalInput) {
      elements.push(
        <td className="option" value={item.getAdditionalInputType()} onChange={e => this.handleChangeOption('additionalInputType', e.target.value)}>
          <select>
      ...
      elements.push(<td className="option"><input className="item-editor-unit" value={item.getUnit()} onChange={e => this.handleChangeOption('unit', e.target.value)} /></td>);
      elements.push(
        <td className="option">
    ...
    elements.push(<td />);
    elements.push(
      <td className="option" style={{ whiteSpace: 'nowrap' }}>
        <input type="checkbox" className="additional-input" onChange={e => this.handleChangeOption('additionalInput', e.target.checked)} checked={item.hasAdditionalInput()} />
      </td>,
    );
    return elements;
  ...
  render() {
    ...
    return connectDragPreview(connectDropTarget(
      <tr className="item-editor-row">
        ...
        {this.renderAdditionlInput()}
        {randomFixed}
        {exclusiveEl}
        <td className="buttons">
          <Glyphicon className="clickable icon-button text-info" glyph="plus-sign" onClick={() => this.handleClickAddButton()} />
          <Glyphicon className="clickable icon-button text-danger" glyph="remove-sign" onClick={() => this.handleClickRemoveButton()} style={controllerRemoveStyle} />
        </td>
      </tr>
    ));
  }
...
const conditionSource = {
  beginDrag(props) {
    // TODO react-dndでやるためにはItemEditorをItemEditorとItemEditorに分けて実装する必要がありそう
    return {
