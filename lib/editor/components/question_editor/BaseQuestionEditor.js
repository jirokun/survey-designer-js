...
class BaseQuestionEditor extends Component {
                ...
                <table className="item-editor-table">
                  <thead>
                    <tr>
                      <th />
                      <th />
                      { question.getDataType() !== 'MultiNumber' && items.find(item => item.hasAdditionalInput()) ? <th>入力<br />タイプ</th> : <th />}
                      { question.getDataType() !== 'MultiNumber' && items.find(item => item.hasAdditionalInput()) ? <th>単位</th> : <th /> }
                      { question.getDataType() !== 'MultiNumber' ? <th>追加<br />入力</th> : <th /> }
  ...
  render() {
    const { state, page, question, noEdit } = this.props;
    return (
      <div>
