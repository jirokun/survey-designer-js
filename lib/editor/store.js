...
export function json2ImmutableState(json) {
  ...
  return Immutable.fromJS(surveyDesignerStateWrapper, (key, value) => {
    switch (key) {
      ...
      case 'pageDefs':
        return value.map(v => new PageDefinition(v));
      case 'questions':
        return value.map(v => new QuestionDefinition(v));
      case 'choices':
        return value.map((v) => {
          if (typeof v === 'string') {
            return new ChoiceDefinition({ label: v, value: v });
          }
          return new ChoiceDefinition(v);
        });
      case 'branchDefs':
        return value.map(v => new BranchDefinition(v));
      case 'conditions':
        return value.map(v => new ConditionDefinition(v));
      case 'childConditions':
        return value.map(v => new ChildConditionDefinition(v));
      case 'flowDefs':
        return value.map(v => new FlowDefinition(v));
      default:
        return value;
    }
  }).get('wrapper');
