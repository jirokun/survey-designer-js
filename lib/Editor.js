...
export function setInitialCssUrls(initialState) {
...
export function Editor(el, json) {
  ...
  if (ENV.RUNTIME_CSS_URL || ENV.PREVIEW_CSS_URL) {
    const runtimeUrls = ENV.RUNTIME_CSS_URL ? ENV.RUNTIME_CSS_URL.split(/,/) : [];
    const previewUrls = ENV.PREVIEW_CSS_URL ? ENV.PREVIEW_CSS_URL.split(/,/) : [];
    if (!json.options.cssOptions) {
