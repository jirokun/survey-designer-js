...
class EnquetePreviewApp extends Component {
  ...
  startAutomaton() {
    ...
    const automaton = new Automaton(survey);
    const node = survey.findNode(runtime.getCurrentNodeId());
    if (node.getType() !== 'page') return;

    const page = runtime.findCurrentPage(survey);
    automaton.start(page);
  }
      ...
      <div>
        ...
        <div className={classNames('preview-main', { open: this.state.answerOpen })}>
          <div className="formButtons preview">
            ...
            <button className="automaton" onClick={() => this.startAutomaton()}>自動入力</button>
          </div>
