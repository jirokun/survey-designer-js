...
import { showOutputNo, showDevId, isDevelopment, appendUrlParams } from '../../utils';
...
import MatrixQuestionJS from '../../runtime/components/plain/MatrixQuestionJS';
import CheckboxQuestionJS from '../../runtime/components/plain/CheckboxQuestionJS';
import RadioQuestionJS from '../../runtime/components/plain/RadioQuestionJS';
import MultiNumberQuestionJS from '../../runtime/components/plain/MultiNumberQuestionJS';
...
class EnqueteDetailApp extends Component {
  componentDidMount() {
    ...
    new MatrixQuestionJS(this.rootEl, survey).initForDetail();
    new CheckboxQuestionJS(this.rootEl, survey).initForDetail();
    new RadioQuestionJS(this.rootEl, survey).initForDetail();
    new MultiNumberQuestionJS(this.rootEl, survey).initForDetail();
    showOutputNo(survey);
  }
      ...
      return (
        <div>
          <div className={classNames('page-no', { hidden: !showPageNo })} data-dev-id-label={page.getDevId()}>
            ...
            {page.hasMatrixHtmlEnabledQuestion() ? <span className="detail-function">テーブルカスタマイズ</span> : null}
  ...
  createSurveySettingDetail() {
    const { survey } = this.props;
    return (
      <div className="survey-setting-detail">
        アンケート設定
        <table>
          <tr>
            <th>ゼロ埋め</th>
            <td>{ survey.getZeroSetting() ? '◯' : null }</td>
          </tr>
        </table>
      </div>
    );
  ...
  render() {
    ...
    return (
      <div className={isDevelopment() && 'development'}>
        ...
        {survey.getCssDetailUrls().toArray().map(url => <link key={url} type="text/css" rel="stylesheet" href={appendUrlParams(url, options.getCacheSuffix())} />)}
        ...
        <div ref={(el) => { this.rootEl = el; }} id="content">
          <div id="surveyBox">
            ...
            { this.createSurveySettingDetail() }
          </div>
          <div>
