...
/** 表示されており、なおかつdisabledがfalseかどうかを確認する */
$.fn.isEnabled = function isEnabled() {
  return this.is(':visible:enabled');
};
...
$.extend($.expr[':'], {
  ...
  /**
   * dev-idでセレクタを指定出来るようにする。
   * @example $(':dev(xxx_yyy)')
   **/
  dev: (elem, idx, args) => {
    const regex = new RegExp(`^${args[3]}$`);
    return regex.test($(elem).data('dev-id'));
  },
});
