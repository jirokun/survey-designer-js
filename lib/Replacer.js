...
export default class Replacer {
  ...
  constructor(allOutputDefinitionMap, questionNoMap, answers) {
    ...
    this.name2ValueParams = Replacer.createName2ValueParams(allOutputDefinitionMap, answers);
    this.name2OutputNoParams = Replacer.createName2OutputNoParams(allOutputDefinitionMap);
    this.outputNo2NameParams = Replacer.createOutputNo2NameParams(questionNoMap);
  }
   ...
   */
  name2Value(str) {
    return Replacer.convert(str, this.name2ValueParams);
  }
   ...
   */
  outputNo2Name(str) {
    return Replacer.convert(str, this.outputNo2NameParams);
  }
   ...
   */
  name2OutputNo(str) {
    return Replacer.convert(str, this.name2OutputNoParams);
  }
  ...
  validate(str) {
    ...
    for (;;) {
      ...
      // 存在している参照かどうかを確かめる
      const key = `${m[1]}.${m[2]}`;
      if (!(key in this.name2ValueParams)) {
        return false;
      }
    }
    return true;
