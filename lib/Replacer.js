...
export default class Replacer {
  ...
  static createId2ValueParams(allOutputDefinitionMap, answers, forEdit) {
    const params = {};
    allOutputDefinitionMap.forEach((od) => {
      params[`{{${od.getId()}.answer}}`] = forEdit ? `<span class="answer-value">${od.getLabelForDetail()}</span>` : answers[od.getName()];
      ...
      if (type === 'radio' || type === 'select') {
        ...
        if (forEdit) {
          params[`{{${od.getId()}.answer_label}}`] = `<span class="answer-value">${od.getLabelForDetail()}</span>`;
        } else {
  ...
  /** 参照しているOutputDefinitionを返す */
  findReferenceOutputDefinitions(str) {
    if (!this.isIncludeName(str)) return [];
